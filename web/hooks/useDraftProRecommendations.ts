...
export function useDraftProRecommendations(input: Input | null, enabled: boolean) {
  const [state, setState] = useState<{ status: "idle" | "loading" | "ready" | "error"; results: RecommendationResult[] | null; error: string | null }>({ status: "idle", results: null, error: null });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        ...
        if (!controller.signal.aborted) setState({ status: "ready", results: payload.data as RecommendationResult[], error: null });
      })().catch((error: unknown) => {
        if (!controller.signal.aborted) setState({ status: "error", results: null, error: error instanceof Error ? error.message : "Draft Pro recommendations are unavailable." });
      });
    }, 200);
