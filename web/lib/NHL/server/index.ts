...
export async function getCurrentSeason(): Promise<Season> {
  const toSeason = (currentSeason: any, lastSeason: any): Season => ({
  ...
  try {
    ...
    const seasons = [...(data ?? [])].sort((a, b) => b.id - a.id);

    if (seasons.length === 0) {
      throw new Error("NHL season feed returned no rows.");
    }

    const now = new Date();
    const currentIndex = seasons.findIndex((season) => {
      const start = new Date(season.startDate);
      const end = new Date(season.regularSeasonEndDate || season.endDate);
