...
import { fetchSeasonGoalieSummaryGameIdSet } from "./goalieStatsSummaryRefresh";
...
export type GoalieStatsIncrementalSelection = {
  mode: "incremental";
  seasonId: number;
  requestedGameType: number;
...
export async function resolveGoalieStatsIncrementalSelection(args?: {
  seasonId?: number | null;
  requestedGameType?: number | null;
  supabase?: SupabaseClient;
}): Promise<GoalieStatsIncrementalSelection> {
  ...
  const summaryGameIds = await fetchSeasonGoalieSummaryGameIdSet({
    supabase,
    seasonId,
  });
