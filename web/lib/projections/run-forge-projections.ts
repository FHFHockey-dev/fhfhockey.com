...
function getConsensusLineValue(
    ...
    gamesLoop: for (const game of (games ?? []) as GameRow[]) {
      ...
      const skaterStageResult = await runPerGameSkaterStage(async () => {
        for (const teamId of [game.homeTeamId, game.awayTeamId]) {
          ...
          const lcContext = await fetchLatestLineCombinationForTeam(
            teamId,
            asOfDate,
          );
          ...
          for (const playerId of teamPositionFilteredSkaterIds) {
            ...
            const latestDate =
              evDate && ppDate
                ...
                : (evDate ?? ppDate);
            if (latestDate)
              latestMetricDateByPlayerId.set(playerId, latestDate);
          }
          ...
          if (skaterIds.length < SKATER_POOL_MIN_VALID_COUNT) {
            ...
            const mergedConstrainedRawSkaterIds =
            ...
            if (mergedUniqueCount > baseUniqueCount) {
              ...
              for (const [
                playerId,
                roleTag,
              ] of fallbackRecoveryRoleTags.entries()) {
          ...
          if (skaterIds.length < SKATER_POOL_MIN_VALID_COUNT) {
            ...
            if (
              emergencyNeeded > 0 &&
              missingMetricEmergencyCandidates.length > 0
            ) {
          ...
          const opponentAbbrev =
            teamAbbreviationById.get(opponentTeamId) ?? null;
          ...
          const teamLevelContextAdjustment =
            computeSkaterTeamLevelContextAdjustments({
              teamStrengthPrior:
              ...
              opponentFiveOnFiveProfile:
                teamFiveOnFiveProfileCache.get(teamDateKey(opponentTeamId)) ??
                null,
              teamNstProfile:
                teamNstExpectedGoalsCache.get(teamDateKey(teamId)) ?? null,
              opponentNstProfile:
                teamNstExpectedGoalsCache.get(teamDateKey(opponentTeamId)) ??
                null,
            });
          ...
          const shotQualityByPlayerId =
            await fetchLatestSkaterShotQualityProfiles(skaterIds, asOfDate);
          const onIceContextByPlayerId =
            await fetchLatestSkaterOnIceContextProfiles(skaterIds, asOfDate);
          ...
          if (trendAdjustmentFetch.diagnostics.fetchFailed) {
            ...
            trendAdjustmentFetch.diagnostics.selectedHardStaleRows > 0
          ) {
            metrics.warnings.push(
          ...
          for (const playerId of skaterIds) {
            ...
            const deploymentPrior =
              deploymentPriorByPlayerId.get(playerId) ?? null;
            const shotQualityProfile =
              shotQualityByPlayerId.get(playerId) ?? null;
            const onIceContextProfile =
              onIceContextByPlayerId.get(playerId) ?? null;
            const toiEsDeploymentPrior =
              deploymentPrior?.esToiPerGameSec ?? null;
            const toiPpDeploymentPrior =
              deploymentPrior?.ppToiPerGameSec ?? null;
            ...
            const assistRateEs = clamp(
              splitRates.assistRateEs * sampleWeight +
                (1 - sampleWeight) * 0.72,
              0.2,
              1.7,
            );
            const assistRatePp = clamp(
              splitRates.assistRatePp * sampleWeight +
                (1 - sampleWeight) * 0.95,
              0.3,
              2,
            );
            ...
            const roleScenarioTopProbability =
              roleScenarios[0]?.probability ?? 1;
            ...
            const latestMetricDate =
              latestMetricDateByPlayerId.get(playerId) ?? null;
            ...
            const trendShotMultiplier =
              trendAdjustment?.shotRateMultiplier ?? 1;
            const trendGoalMultiplier =
              trendAdjustment?.goalRateMultiplier ?? 1;
            ...
            projected.set(playerId, {
              ...
              assistRateEs: clamp(
                assistRateEs * trendAssistMultiplier,
                0.2,
                1.7,
              ),
              ...
              eventAvailabilityMultiplier,
              availabilityEvent:
                availabilityEventByPlayer.get(playerId) ?? null,
              roleEvent,
              latestMetricDate,
              daysSinceLastMetric,
              recencyMultiplier,
          ...
          if (roleScenarioPlayersModeled > 0) {
            ...
            const avgCount =
              roleScenarioCountTotal / roleScenarioPlayersModeled;
            metrics.data_quality.role_scenarios_avg_count =
              metrics.data_quality.role_scenarios_avg_count == null
                ? avgCount
                : (metrics.data_quality.role_scenarios_avg_count + avgCount) /
                  2;
          }
          ...
          for (const [playerId, p] of projected.entries()) {
              ...
              model: {
                ...
                skater_selection: {
                  ...
                  source_rows: {
                    ...
                    roster_event_id:
                      p.roleEvent?.event_id ??
                      p.availabilityEvent?.event_id ??
                      null,
                  },
                  fallback_path: {
                  ...
                  active_pool: {
                    raw_candidate_count: Array.from(new Set(rawSkaterIds))
                      .length,
                  ...
                  role_continuity: p.roleContinuity
                    ? {
                        ...
                        appearances_tracked:
                          p.roleContinuity.appearancesTracked,
                  ...
                  role_scenarios: {
                    ...
                    scenario_stat_lines: scenarioBlend.scenarioLines.map(
                      (s) => ({
                        role: s.role,
                        probability: Number(s.probability.toFixed(4)),
                    ...
                    horizon_summaries:
                      scenarioBlend.horizonScenarioSummaries.map((s) => ({
                  ...
                  shot_quality: {
                    ...
                    shot_rate_multiplier: Number(
                      p.shotRateMultiplier.toFixed(4),
                    ),
                    goal_rate_multiplier: Number(
                      p.goalRateMultiplier.toFixed(4),
                    ),
                  ...
                  on_ice_context: {
                    ...
                    sample_weight: Number(
                      p.onIceContextSampleWeight.toFixed(4),
                    ),
                  ...
                  rest_schedule: {
                    sample_weight: Number(
                      p.restScheduleSampleWeight.toFixed(4),
                    ),
                  ...
                  small_sample: {
                    ...
                    evidence_shots: Number(
                      p.smallSampleEvidenceShots.toFixed(3),
                    ),
                  },
                  conversion_rates: {
                  ...
                  role_usage_bounds: {
                    ...
                    bounded_sog_per_60_es: Number(
                      p.boundedSogPer60Es.toFixed(3),
                    ),
                    bounded_sog_per_60_pp: Number(
                      p.boundedSogPer60Pp.toFixed(3),
                    ),
                  },
                },
              },
            ...
            playerUpserts.push({
              ...
              proj_shots_es: Number(
                (shotsEs * teamHorizonTotalScalar).toFixed(3),
              ),
              proj_shots_pp: Number(
                (shotsPp * teamHorizonTotalScalar).toFixed(3),
              ),
              proj_shots_pk: null,
              proj_goals_es: Number(
                (goalsEs * teamHorizonTotalScalar).toFixed(3),
              ),
              proj_goals_pp: Number(
                (goalsPp * teamHorizonTotalScalar).toFixed(3),
              ),
              proj_goals_pk: null,
              ...
              proj_blocks: Number(
                (projBlocks * teamHorizonTotalScalar).toFixed(3),
              ),
          ...
          for (const [
            ...
            const marketSummaryByType =
              playerPropContextByGamePlayerKey.get(`${game.id}:${playerId}`) ??
              null;
            ...
            for (const [marketType, marketSummary] of Object.entries(
              ...
              playerPredictionOutputRows.push({
                ...
                components: {
                  ...
                  projection_inputs: {
                    lineup_source_game_date:
                      projectionInput.lineupSourceGameDate,
                ...
                metadata: {
                  ...
                  availability_event: projectionInput.availabilityEvent
                    ? {
                        event_type:
                          projectionInput.availabilityEvent.event_type,
                        confidence:
                          projectionInput.availabilityEvent.confidence ?? null,
                        effective_from:
                          projectionInput.availabilityEvent.effective_from ??
                          null,
                        effective_to:
                          projectionInput.availabilityEvent.effective_to ??
                          null,
                      }
                    : null,
                },
              ...
              const edgeThreshold =
                PLAYER_MARKET_EDGE_THRESHOLDS[marketType] ?? 0.25;
          ...
          for (const row of goalieStarts ?? []) {
            ...
            const projectedGsaaPer60 = Number(
              (row as any)?.projected_gsaa_per_60,
            );
          ...
          const contextGoalies = Array.from(
            context.startsByGoalie.keys(),
          ).slice(0, 4);
      ...
      const goalieStageResult = await runPerGameGoalieStage(async () => {
        for (const c of goalieCandidates) {
          ...
          const defendingTeamAbbrev =
            teamAbbreviationById.get(c.teamId) ?? null;
          ...
          const teamStrengthContextAdjustment =
            computeTeamStrengthContextAdjustment({
          ...
          const opponentFiveOnFiveProfile =
            teamFiveOnFiveProfileCache.get(teamDateKey(c.opponentTeamId)) ??
            null;
          ...
          const opponentNstExpectedGoalsProfile =
            teamNstExpectedGoalsCache.get(teamDateKey(c.opponentTeamId)) ??
            null;
          const nstOpponentDangerAdjustment =
            computeNstOpponentDangerAdjustment({
          ...
          const blendedShotsAgainst =
            teamSaAvg10 != null
              ? 0.65 * opponentProjectedShotsAgainst +
                0.35 * teamSaAvg10 +
                trendAdj
          ...
          const shotsTrendPct =
            baseShotsAgainst > 0 && oppShots5 != null
              ? clamp(
                  (oppShots5 - baseShotsAgainst) / baseShotsAgainst,
                  -0.12,
                  0.18,
                )
              : 0;
          ...
          if (c.override) {
            ...
            const opponentGoalsForRaw =
              teamGoalsByTeamId.get(c.opponentTeamId) ?? 0;
            const opponentGoalsFor = Number(
              Math.max(
                0,
                opponentGoalsForRaw *
                  (1 +
                    teamStrengthContextAdjustment.opponentGoalsForPctAdjustment),
              ).toFixed(3),
            );
            const teamIsWeaker =
              adjustedTeamGoalsFor + TEAM_STRENGTH_WEAKER_GAP <
              opponentGoalsFor;
            const opponentIsWeak =
              opponentGoalsFor <= WEAK_OPPONENT_GF_THRESHOLD;
            const isB2B =
            ...
            const ranked = Array.from(probs.entries()).sort(
              (a, b) => b[1] - a[1],
            );
          ...
          if (
            !goalieWorkloadContextCache.has(playerDateKey(selectedGoalieId))
          ) {
          ...
          if (
            !goalieRestSplitProfileCache.has(playerDateKey(selectedGoalieId))
          ) {
          ...
          const restSplitProfile =
            goalieRestSplitProfileCache.get(playerDateKey(selectedGoalieId)) ??
            null;
          const workloadSavePctPenalty =
            computeWorkloadSavePctPenalty(workload);
          const restSplitSavePctAdjustment =
            computeGoalieRestSplitSavePctAdjustment({
          ...
          for (const scenario of topStarterScenarios) {
            ...
            if (
              !goalieWorkloadContextCache.has(playerDateKey(scenario.goalieId))
            ) {
              goalieWorkloadContextCache.set(
                playerDateKey(scenario.goalieId),
            ...
            if (
              !goalieRestSplitProfileCache.has(playerDateKey(scenario.goalieId))
            ) {
              goalieRestSplitProfileCache.set(
                playerDateKey(scenario.goalieId),
            ...
            const scenarioRestSplitProfile =
              goalieRestSplitProfileCache.get(
                playerDateKey(scenario.goalieId),
              ) ?? null;
            ...
            const scenarioLeagueSavePct = clamp(
              leagueSavePct -
                scenarioWorkloadPenalty +
                scenarioRestSplitAdjustment,
              0.86,
              0.92,
            );
            ...
            scenarioProjections.push({
              ...
              starter_probability_raw: Number(
                scenario.rawProbability.toFixed(4),
              ),
          ...
          const blendedProjection = blendTopStarterScenarioOutputs({
            ...
            fallbackProjection: {
              ...
              workload_save_pct_penalty: Number(
                workloadSavePctPenalty.toFixed(4),
              ),
              rest_split_save_pct_adjustment: Number(
              ...
              ? [
                  {
                    ...
                    shotsAgainst,
                    goalsAllowed: Number(
                      selectedGoalieFullStartModel.projectedGoalsAllowed.toFixed(
                        3,
                      ),
                    ),
                    saves: Number(
          ...
          const goalieUpsert = {
            ...
            proj_win_prob: Number(
              (winProb * goalieHorizonTotalScalar).toFixed(4),
            ),
            proj_shutout_prob: Number(
          ...
          const goalieMarketSummaryByType =
            playerPropContextByGamePlayerKey.get(
              `${game.id}:${selectedGoalieId}`,
            ) ?? null;
              ...
              playerPredictionOutputRows.push({
                ...
                components: {
                  ...
                  projection_inputs: {
                    ...
                    confirmed_status:
                      c.override != null ||
                      c.confirmedStarterByGoalieId.get(selectedGoalieId) ===
                        true,
                  },
                },
                provenance: {
