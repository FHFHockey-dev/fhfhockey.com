...
export function buildWindowPayload(rows: GameRow[]): WindowPayload {
  ...
  for (const m of metrics) {
    ...
    if (m === "shooting_percentage") {
      ...
      values = rows.map((r) =>
        pct(safeNumber(r.goals_5v5 || 0), safeNumber(r.shots || 0))
      );
      ...
      // per-60 using nst_toi when available
      const numerKey = m.replace(/_per_60$/, "");
      values = rows.map((r) =>
        per60(
          safeNumber(r[numerKey] || 0),
          safeNumber(r.nst_toi || r.toi_per_game || 0)
        )
      );
    } else if (m === "shots") {
    ...
    if (m === "shooting_percentage_5v5") {
      const goalsSum = rows.reduce(
        (s, r) => s + safeNumber(r.goals_5v5 || 0),
        0
      );
      const shotsSum = rows.reduce((s, r) => s + safeNumber(r.shots || 0), 0);
      payload[m].numer = goalsSum;
    ...
    if (m.endsWith("_per_60")) {
      // attempt to store summed numerators and toi_seconds so downstream EB can reuse
      const numerKey = m.replace(/_per_60$/, "");
      // special-case PP keys
      const ppMap: Record<string, string> = {
        pp_points_per_60: "pp_points",
        pp_goals_per_60: "pp_goals",
        pp_shots_per_60: "pp_shots"
      };
      const actualNumerKey = ppMap[m] || numerKey;
      const numerSum = rows.reduce(
        (s, r) => s + safeNumber(r[actualNumerKey] || 0),
        0
      );
      const toiSum = rows.reduce(
        (s, r) => s + safeNumber(r.nst_toi || r.toi_per_game || r.pp_toi || 0),
        0
      );
      payload[m].numer = numerSum;
      payload[m].denom = toiSum;
      payload[m].toi_seconds = toiSum;
    }
  }
...
export function computePPControls(
  ...
  const L10 = recentRows.slice(0, 10);
  const L20 = recentRows.slice(0, 20);
  ...
  const pp1_flag = pp_share_sm >= deploymentElasticity.pp1Threshold;
