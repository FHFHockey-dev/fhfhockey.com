...
const candidates = [
  { id: "candidate-b", name: "Candidate B", teamAbbreviation: "BBB", eligiblePositions: "RW", value: 10, projectionSeason: season },
...
function input(overrides: Partial<DraftProDustInput> = {}): DraftProDustInput {
  return {
    ...
    schedule: { season, fetchedAt: now.toISOString(), games },
    ...overrides,
  };
}
...
describe("Draft Pro DUST", () => {
  ...
  it("sorts equal schedule-fit results by existing value then stable id", () => {
    expect(evaluateDraftProDust(input(), now).insights.map((item) => item.playerId)).toEqual(["candidate-a", "candidate-b"]);
  });
  ...
  it("rejects projection or schedule season boundaries instead of mixing them", () => {
    ...
    expect(evaluateDraftProDust(input({ schedule: { season, fetchedAt: now.toISOString(), games: [{ ...games[0], season: "2025" }] } }), now).state).toBe("season_mismatch");
  });

  it("fails closed for stale schedules and weekly-lock leagues", () => {
    expect(evaluateDraftProDust(input({ schedule: { season, fetchedAt: new Date(now.getTime() - DRAFT_PRO_DUST_STALE_AFTER_MS - 1).toISOString(), games } }), now).state).toBe("stale_schedule");
