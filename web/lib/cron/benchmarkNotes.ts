...
const BENCHMARK_NOTES_BY_JOB: Record<string, BenchmarkAnnotation[]> = {
  "update-nst-gamelog": [
    {
      kind: "bottleneck",
      note: "Direct NST scraper with date iteration and enforced inter-request pacing.",
    },
    {
      kind: "rate_limited",
  ...
  "update-rolling-player-averages": [
    ...
    },
    {
      ...
      note: "Downstream FORGE and Start Chart freshness depend on this output; local remediation validation now succeeds within budget.",
    },
  ],
  ...
  "ingest-projection-inputs": [
    {
      kind: "stateful",
      note: "Recent-game ingest reads multiple upstream tables, but the bounded resumable path now returns actionable progress cleanly.",
    },
    {
      kind: "stateful",
  ...
  "build-forge-derived-v2": [
    {
      kind: "stateful",
      note: "Runs multiple derived-table builders in one request, but the per-date bounded path now completes cleanly in local validation.",
    },
    {
      kind: "dependency_sensitive",
  ...
  "run-forge-projection-v2": [
    {
      kind: "bottleneck",
      note: "Preflight no longer blocks the route incorrectly; the remaining blocker is execution-time DB statement timeout risk.",
    },
    {
      kind: "dependency_sensitive",
  ...
  "update-nst-goalies": [
    {
      kind: "rate_limited",
      note: "Direct NST scraper; small bounded runs now burst only when the computed request budget stays under the shared 80-page/5-minute and 180-page/hour NST key ceilings.",
    },
    {
      kind: "stateful",
  ...
  "update-nst-player-reports": [
    {
      kind: "rate_limited",
      note: "Direct NST scraper; this route also spends the shared NST key budget even when run manually or as a targeted repair path.",
    },
    {
      kind: "stateful",
  ...
  "update-nst-last-ten": [
    {
      kind: "rate_limited",
      note: "Direct NST scraper; treat it as another consumer of the shared NST key budget rather than an isolated helper route.",
    },
    {
      kind: "stateful",
  ...
  "check-missing-goalie-data": [
    {
      kind: "rate_limited",
      note: "Direct NST verification route; manual repair runs still draw from the same shared NST key budget as scheduled jobs.",
    },
    {
      kind: "stateful",
  ...
  "update-nst-team-daily": [
    {
      kind: "rate_limited",
      note: "Direct NST route; burst mode is now selected from request-count math under the shared NST key budget rather than date-count assumptions.",
    },
    {
      kind: "stateful",
  ...
  "update-nst-team-daily-incremental": [
    {
      kind: "rate_limited",
      note: "Scheduled direct NST route; it shares the same NST key budget as gamelog, goalies, current-season, and team-stats jobs.",
    },
    {
      kind: "stateful",
  ...
  "update-nst-team-stats-all": [
    {
      kind: "rate_limited",
      note: "Direct NST team-stats route; small compliant runs can now burst when they stay inside the shared NST key budget instead of paying the legacy fixed 21s gap.",
    },
    {
      kind: "stateful",
  ...
  "update-wigo-table-stats": [
    {
      kind: "bottleneck",
      note: "Route still appears long-running after remediation; local validation did not complete within the 180s probe window.",
    },
  ],
  "update-season-stats-current-season": [
    {
      kind: "bottleneck",
      note: "Route still hangs past the 180s validation probe, so the current issue is long-running execution rather than HTML error leakage.",
    },
  ],
  "update-sko-stats-full-season": [
    {
      kind: "dependency_sensitive",
      note: "Historical schema drift is contained by the canonical 28-column sko_skater_stats writer projection; retain this dependency annotation until a natural Production receipt confirms the scheduled full-season path.",
    },
  ],
  "update-wgo-averages": [
    {
      kind: "dependency_sensitive",
      note: "Current blocker is a structured upstream transport/dependency failure rather than raw HTML leakage.",
    },
  ],
  "run-projection-accuracy": [
    {
      kind: "dependency_sensitive",
      note: "Current failure mode is now cleanly downstream-dependent on a succeeded projection run for the target date.",
    },
  ],
};

export function getBenchmarkAnnotations(
  jobName: string,
): BenchmarkAnnotation[] {
