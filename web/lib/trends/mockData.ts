...
import { buildBaselineDensity, seasonStringToId } from "./utils";
...
export function buildMockPlayerSeries(
  ...
  const timeSeries: TimeSeriesPoint[] = Array.from(
    ...
    (_, idx) => {
      ...
      const secondaryWave =
        Math.cos(idx / 4 + primaryPhase) * (amplitude * 0.45);
  ...
  return {
    ...
    baseline: {
      ...
      nTrain: 196,
      density: buildBaselineDensity(baselineMu, baselineSigma)
    },
    timeSeries,
    isMock: true
  };
