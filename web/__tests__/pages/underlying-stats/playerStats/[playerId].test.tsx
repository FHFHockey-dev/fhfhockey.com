...
describe("PlayerUnderlyingStatsDetailPage", () => {
  ...
  beforeEach(() => {
    window.sessionStorage.clear();
    routerMock.pathname = "/underlying-stats/playerStats/[playerId]";
    routerMock.query = {
      playerId: "8478401",
  ...
  it("renders goalie-first detail framing and uses the goalie API namespace for the goalie variant", async () => {
    ...
    render(<PlayerUnderlyingStatsDetailPage variant="goalie" />);

    await waitFor(() => {
      expect(screen.getByText("Goalie Detail")).toBeTruthy();
    });
    ...
    expect(screen.getByText("Goalie Stats")).toBeTruthy();
    expect(screen.getAllByText("Goalie 8475883").length).toBeGreaterThan(0);
    expect(screen.getByText("Route Goalie")).toBeTruthy();
    expect(screen.getByText("Initial Goalie Table")).toBeTruthy();
    expect(screen.getByText("Goalie Detail Controls")).toBeTruthy();
    expect(screen.getByText("2025-26 · Igor Shesterkin")).toBeTruthy();
    ...
    expect(requestUrl).toContain("/api/v1/underlying-stats/goalies/8475883?");
    expect(requestUrl).toContain("againstTeamId=6");
    expect(requestUrl).not.toContain("/api/v1/underlying-stats/players/");
  });
