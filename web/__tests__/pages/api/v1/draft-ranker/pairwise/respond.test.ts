...
vi.mock("lib/draft-ranker/server", () => ({ submitDraftPairComparison: submitMock }));
...
describe("/api/v1/draft-ranker/pairwise/respond", () => {
  ...
  it("submits only the bounded outcome contract for the authenticated owner", async () => {
    ...
    const res = response();
    await handler({ method: "POST", headers: {}, query: { userId: "forged" }, body } as any, res as any);
    expect(submitMock).toHaveBeenCalledWith("owner-1", body);
    expect(res.statusCode).toBe(200);

    const forged = response();
    await handler({ method: "POST", headers: {}, query: {}, body: { ...body, preferredPlayerId: 99 } } as any, forged as any);
  ...
  it("rejects arbitrary outcomes and unsupported methods", async () => {
    ...
    await handler({ method: "GET", headers: {}, query: {} } as any, method as any);
