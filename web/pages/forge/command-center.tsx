...
} from "lib/dashboard/commandCenterData";
import {
  buildForgeHref,
  parseForgeDateParam,
  parseForgeModeParam,
...
const ForgeCommandCenterPage: NextPage = () => {
  ...
  const [commandData, setCommandData] = useState<CommandCenterData | null>(
    null,
  );
  ...
  const requestedState = useMemo(() => {
    ...
    const normalizedTeam =
      requestedTeam &&
      requestedTeam !== "ALL" &&
      teamOptions.includes(requestedTeam)
        ? requestedTeam
        : "all";
  ...
  useEffect(() => {
    ...
    loadCommandCenterData(routeState)
      .then((data) => {
        ...
        setCommandData(data);
        if (
          data.routeState.resolvedDate &&
          data.routeState.resolvedDate !== resolvedDate
        ) {
  ...
  return (
    <>
      ...
      >
        ...
        {commandData ? (
          <MixedStateBanner mixedState={commandData.mixedState} />
        ) : null}
        {dataError ? (
          <p className={styles.moduleStateWarning}>{dataError}</p>
        ) : null}
