...
export default function PlayerStatsPage({
  ...
  const filteredGameLog = useMemo(() => {
    ...
    const games = timeFrame === "last10" ? 10 : timeFrame === "last20" ? 20 : 30;
  ...
  const processedGameLog = useMemo(() => {
    return filteredGameLog.map((game: GameLogEntry, index: number) => {
      ...
      selectedStats.forEach((stat) => {
        const values = rollingWindow.map((g: GameLogEntry) => Number(g[stat]) || 0);
    ...
    <Layout>
      ...
      <div className={styles.playerStatsPageContainer}>
        ...
        <div className={styles.tabNavigation}>
          {(["overview", "advanced", "trends", "calendar"] as StatsTab[]).map((tab) => (
            <button
              key={tab}
          ...
          <div className={styles.controlsGrid}>
            ...
            <div className={styles.timeframeSelector}>
              ...
              <div className={styles.timeframeButtons}>
                {(["season", "last10", "last20", "last30"] as TimeFrame[]).map((frame) => (
                  <button
                    key={frame}
                    onClick={() => setTimeFrame(frame)}
                    className={`${styles.timeframeButton} ${timeFrame === frame ? styles.active : ""}`}
                  >
                    {frame === "season" ? "Full Season" : 
                     frame === "last10" ? "Last 10" :
                     frame === "last20" ? "Last 20" : "Last 30"}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className={styles.advancedGrid}>
              <div className={styles.advancedTable}>
                <PlayerStatsTable
                  gameLog={filteredGameLog}
                  playoffGameLog={playoffGameLog}
                  selectedStats={positionConfig.advanced || positionConfig.secondary}
                  isGoalie={isGoalie}
                  showAdvanced={true}
                  showPlayoffData={showPlayoffData}
                  playerId={Number(playerId)}
...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  if (isGoalie) {
    ...
    if (data) {
      ...
      mostRecentSeason = seasonTotals.length > 0 ? (seasonTotals[0] as GoalieSeasonTotals).season_id ?? null : null;
    }
  } else {
