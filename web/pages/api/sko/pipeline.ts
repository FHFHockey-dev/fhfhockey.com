...
const SENSITIVE_KEYS = new Set([
  'secret','token','authorization','sko_pipeline_secret','api_key'
]);
...
function checkAuth(req: NextApiRequest): [boolean, string] {
  ...
  const token = auth.split(' ', 1)[0] === 'Bearer' ? auth.slice(7) : auth.split(' ')[1];
...
async function postStep(endpoint: string, headers: Record<string,string>, payload: any, step: string): Promise<StepResult> {
  ...
  try {
    ...
    const duration_sec = Number(((performance.now() - started)/1000).toFixed(2));
    let data: any = {};
    ...
    return {
      ...
      response: data,
      message: data?.message || data?.error || `${step} step ${ok? 'completed':'failed'}`,
    };
  } catch (err:any) {
    const duration_sec = Number(((performance.now() - started)/1000).toFixed(2));
    return {
...
export default async function handler(
  ...
  if (!payload.step && !payload.steps) {
    ...
    else if (typeof req.query.steps === 'string') payload.steps = req.query.steps.split(',').map(s=>s.trim()).filter(Boolean);
  }

  const endpoint = process.env.SKO_PIPELINE_ENDPOINT;
  if (!endpoint) {
    return res.status(500).json({ success: false, message: 'SKO_PIPELINE_ENDPOINT not configured' });
  }
  ...
  for (const step of steps) {
    ...
    if (stepResult.status !== 'ok') { overallSuccess = false; break; }
  }
  const message = results.length ? results[results.length-1].message : 'No pipeline steps executed';
  const total_duration_sec = Number(results.reduce((a,r)=>a + (r.duration_sec||0),0).toFixed(2));
