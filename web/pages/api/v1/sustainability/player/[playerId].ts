...
import {
  getPlayerSustainabilityPayload,
  getPlayerSustainabilitySummaryPayload
} from "lib/sustainability/read";

const WINDOWS = new Set([3, 5, 10, 20]);
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const summary = ["1", "true", "yes"].includes(
    first(req.query.summary)?.toLowerCase() ?? ""
  );
  ...
  if (!WINDOWS.has(window) || !HORIZONS.has(horizon)) {
    return res.status(400).json({
      success: false,
      message: "window must be 3/5/10/20 and horizon must be 5/10"
    });
  }

  try {
    const payload = summary
      ? await getPlayerSustainabilitySummaryPayload({
          client: supabase,
          playerId
        })
      : await getPlayerSustainabilityPayload({
          client: supabase,
          playerId,
          window,
          horizon
