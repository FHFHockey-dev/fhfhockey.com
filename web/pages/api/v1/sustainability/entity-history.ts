...
function parseLimit(value: string | string[] | undefined, fallback = 180) {
  const candidate = Array.isArray(value) ? value[0] : value;
...
export default async function handler(
  ...
  try {
    ...
    const payload = await fetchSandboxScoreHistory({
      entityType,
      entityId,
      requestedDate,
      windowCode,
    ...
    return res.status(500).json({
      success: false,
      message: error?.message ?? "Failed to load sustainability history"
    });
  }
}
