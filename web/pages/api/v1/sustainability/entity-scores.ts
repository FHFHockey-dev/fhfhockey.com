...
function parseLimit(value: string | string[] | undefined, fallback = 60) {
  const candidate = Array.isArray(value) ? value[0] : value;
...
export default async function handler(
  ...
  try {
    ...
    const payload = await fetchSandboxScores({
      entityType,
      entityId,
      requestedDate,
      windowCode,
    ...
    return res.status(500).json({
      success: false,
      message: error?.message ?? "Failed to load sustainability scores"
    });
  }
}
