...
type UpdateGoalieUnderlyingStatsResponse =
  | {
      success: true;
      route: string;
...
async function handler(
  req: NextApiRequest,
  res: NextApiResponse<UpdateGoalieUnderlyingStatsResponse>
) {
  ...
  try {
    ...
    if (processedGameIds.length === 0 && failures.length > 0) {
      return res.status(502).json({
        success: false,
        error: "Goalie underlying stats ingest failed for every requested game.",
    ...
    return res.status(200).json({
      ...
      message:
        failures.length === 0
          ...
          : "Goalie underlying stats ingest completed with partial failures. Successful games were refreshed and summarized; failed games can be retried with the same URL.",
    });
  } catch (error) {
    const message = (() => {
      ...
      return "Unable to update goalie underlying stats.";
    })();
...
export default withCronJobAudit(adminOnly(handler), {
  jobName: "/api/v1/db/update-goalie-underlying-stats",
});