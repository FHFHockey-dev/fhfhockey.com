...
import { refreshPlayerUnderlyingSummarySnapshotsForGameIds } from "lib/underlying-stats/playerStatsSummaryRefresh";
...
async function handler(req: NextApiRequest, res: NextApiResponse<CatchUpResponse>) {
  ...
  try {
    ...
    const selection = await resolvePlayerStatsIncrementalSelection({
      seasonId: parsePositiveInteger(req.query.seasonId),
      requestedGameType: parsePositiveInteger(req.query.gameType),
    });

    console.info("[catch-up-player-underlying] selection", {
    ...
    if (selection.gameIds.length === 0) {
      return res.status(200).json({
        success: true,
        route: "/api/v1/db/catch-up-player-underlying",
        mode: "incremental",
        seasonId: selection.seasonId,
        startDate: selection.startDate,
        endDate: selection.endDate,
        ...
        results: [],
        message: "Player underlying catch-up is already current for the selected season.",
      });
    }
    ...
    console.info("[catch-up-player-underlying] complete", {
      seasonId: selection.seasonId,
      requestedGameCount: selection.gameIds.length,
    ...
    if (batchResult.processedGameIds.length === 0 && batchResult.failures.length > 0) {
      return res.status(502).json({
        success: false,
        error: "Player underlying catch-up failed for every requested game.",
    ...
    return res.status(200).json({
      ...
      message:
        batchResult.failures.length === 0
          ? "Player underlying catch-up completed through the latest finished games."
          : "Player underlying catch-up completed with partial failures.",
    });
  } catch (error) {
    const { message, stack } = formatUnknownError(error);
    const failureStage = typeof stage === "string" ? stage : "unknown";
    console.error("[catch-up-player-underlying] failed", {
      stage: failureStage,
      query: req.query,
      error: message,
      stack,
    ...
    return res.status(500).json({
      success: false,
      error: `Player underlying catch-up failed during ${failureStage}: ${message}`,
      issues: [
        `stage=${failureStage}`,
        message,
      ],
...
export default withCronJobAudit(adminOnly(handler), {
  jobName: "/api/v1/db/catch-up-player-underlying",
});