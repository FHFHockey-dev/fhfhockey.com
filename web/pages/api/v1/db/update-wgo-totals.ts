...
async function fetchAllTotalsForSeason(season: string, limit: number): 
    Promise<{ 
    ...
    while (moreDataAvailable) {
        ...
        const [ skaterTotalStatsResponse,
            ...
            = await Promise.all([
                Fetch(skaterTotalStatsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(miscSkaterTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(faceOffTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(faceoffWinLossTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(goalsForAgainstTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(penaltiesTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(penaltyKillTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(powerPlayTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(puckPossessionTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(satCountsTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(satPercentagesTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(scoringRatesTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(scoringPerGameTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(shotTypeTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
                Fetch(timeOnIceTotalsUrl).then(res => res.json() as Promise<NHLApiResponse>),
            ]);

        // Concatenate the fetched data to the accumulated array
        skaterTotalStats = skaterTotalStats.concat(skaterTotalStatsResponse.data as WGOSummarySkaterTotal[]);
        miscTotalSkaterStats = miscTotalSkaterStats.concat(miscSkaterTotalsResponse.data as WGORealtimeSkaterTotal[]);
        faceOffTotalStats = faceOffTotalStats.concat(faceOffTotalsResponse.data as WGOFaceoffSkaterTotal[]);
        faceoffWinLossTotalStats = faceoffWinLossTotalStats.concat(faceoffWinLossTotalsResponse.data as WGOFaceOffWinLossSkaterTotal[]);
        goalsForAgainstTotalStats = goalsForAgainstTotalStats.concat(goalsForAgainstTotalsResponse.data as WGOGoalsForAgainstSkaterTotal[]);
        penaltiesTotalStats = penaltiesTotalStats.concat(penaltiesTotalsResponse.data as WGOPenaltySkaterTotal[]);
        penaltyKillTotalStats = penaltyKillTotalStats.concat(penaltyKillTotalsResponse.data as WGOPenaltyKillSkaterTotal[]);
        powerPlayTotalStats = powerPlayTotalStats.concat(powerPlayTotalsResponse.data as WGOPowerPlaySkaterTotal[]);
        puckPossessionTotalStats = puckPossessionTotalStats.concat(puckPossessionTotalsResponse.data as WGOPuckPossessionSkaterTotal[]);
        satCountsTotalStats = satCountsTotalStats.concat(satCountsTotalsResponse.data as WGOSatCountSkaterTotal[]);
        satPercentagesTotalStats = satPercentagesTotalStats.concat(satPercentagesTotalsResponse.data as WGOSatPercentageSkaterTotal[]);
        scoringRatesTotalStats = scoringRatesTotalStats.concat(scoringRatesTotalsResponse.data as WGOScoringRatesSkaterTotal[]);
        scoringPerGameTotalStats = scoringPerGameTotalStats.concat(scoringPerGameTotalsResponse.data as WGOScoringCountsSkaterTotal[]);
        shotTypeTotalStats = shotTypeTotalStats.concat(shotTypeTotalsResponse.data as WGOShotTypeSkaterTotal[]);
        timeOnIceTotalStats = timeOnIceTotalStats.concat(timeOnIceTotalsResponse.data as WGOToiSkaterTotal[]);
        ...
        moreDataAvailable = skaterTotalStatsResponse.data?.length === limit
            && miscSkaterTotalsResponse.data?.length === limit
    ...
    console.log(`Fetched ${skaterTotalStats.length} skater stats for season ${season}`);
    console.log(`Fetched ${miscTotalSkaterStats.length} misc skater stats for season ${season}`);
    console.log(`Fetched ${faceOffTotalStats.length} faceoff skater stats for season ${season}`);
    console.log(`Fetched ${faceoffWinLossTotalStats.length} faceoff win/loss skater stats for season ${season}`);
    console.log(`Fetched ${goalsForAgainstTotalStats.length} goals for/against skater stats for season ${season}`);
    console.log(`Fetched ${penaltiesTotalStats.length} penalty skater stats for season ${season}`);
    console.log(`Fetched ${penaltyKillTotalStats.length} penalty kill skater stats for season ${season}`);
    
}

// Function to update skater stats for a season date in the Supabase database
async function updateSkaterTotals(season: string): 
    Promise<{ 
        updated: boolean; 
        playerStats: Record<number, any>;
    }> {

    const { skaterTotalStats,
            miscTotalSkaterStats,
            faceOffTotalStats,
            faceoffWinLossTotalStats,
            goalsForAgainstTotalStats,
    ...
    for (const stat of skaterTotalStats) {
        ...
        const miscTotals = miscTotalSkaterStats.find(aStat => aStat.playerId === stat.playerId);
        const faceOffTotals = faceOffTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const faceoffWinLossTotals = faceoffWinLossTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const goalsForAgainstTotals = goalsForAgainstTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const penaltiesTotals = penaltiesTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const penaltyKillTotals = penaltyKillTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const powerPlayTotals = powerPlayTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const puckPossessionTotals = puckPossessionTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const satCountsTotals = satCountsTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const satPercentagesTotals = satPercentagesTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const scoringRatesTotals = scoringRatesTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const scoringPerGameTotals = scoringPerGameTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const shotTypeTotals = shotTypeTotalStats.find(aStat => aStat.playerId === stat.playerId);
        const timeOnIceTotals = timeOnIceTotalStats.find(aStat => aStat.playerId === stat.playerId);
        ...
        await supabase.from("wgo_skater_stats_totals").upsert({
            ...
            individual_shots_for_per_60: puckPossessionTotals?.individualShotsForPer60, // float
            ...
            secondary_assists_per_60_5v5: scoringRatesTotals?.secondaryAssistsPer605v5, // float
