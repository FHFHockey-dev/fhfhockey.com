...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const view = first(req.query.view);
  if (!Number.isInteger(seasonId) || seasonId <= 0 || !["opening", "current"].includes(view)) {
  ...
  try {
    const payload = await loadFantasyProjectionTeams({
      supabase: getServiceRoleClient(),
      seasonId,
      view: view as "opening" | "current",
    });
