...
import { pollEspnDraftSession } from "lib/integrations/espn/liveDraftServer";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    return res.status(200).json(
      await pollEspnDraftSession({ userId: user.id, sessionId: sessionId(req) }),
    );
  } catch (error) {
    return respondWithEspnError(res, error, "ESPN draft poll failed.");
  }
}
