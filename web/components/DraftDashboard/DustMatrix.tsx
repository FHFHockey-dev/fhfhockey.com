    ...
    <div id={contentId} hidden={!open}>
      {!ready ? <p role="status">{error ?? (state.status === "loading" || !weeks.length ? "Loading weekly schedule…" : state.error ?? "Complete schedule and eligibility data are required for the matrix.")}</p> : !rows.length ? <p>Draft players to see weekly lineup conflicts.</p> : <>
        ...
        <div className={styles.viewport} ref={viewportRef}>
          ...
          <div className={styles.matrixFooter} aria-label="Benched games by matchup week"><strong>Benched games</strong>{weeks.map((week) => <span key={week.week} className={selectedWeekNumbers.has(week.week) ? styles.selectedWeek : undefined} title={state.baseline?.complete === false ? `Week ${week.week}: unavailable` : `Week ${week.week}: ${weekTotals.get(week.week) ?? 0} benched games`}>W{week.week} {state.baseline?.complete === false ? "Unavailable" : weekTotals.get(week.week) ?? 0}</span>)}</div>
        </div>
