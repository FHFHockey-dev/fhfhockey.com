...
export default function DraftProPanel() {
  ...
  async function redeemAccessCode(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const refreshedAccount = await loadAccount(epoch);
      if (!checkoutMountedRef.current || epoch !== accountEpochRef.current || identity !== accountIdentityRef.current || !refreshedAccount) return;
