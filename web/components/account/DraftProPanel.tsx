...
export default function DraftProPanel() {
  ...
  return <section className={styles.panel} aria-labelledby="draft-pro-panel-title">
    ...
    {!account.access.eligible ? <><div className={styles.actions}>{checkoutAvailability.available ? <button type="button" className={styles.premiumButton} onClick={() => void startCheckout()} disabled={checkoutLoading}>{checkoutLoading ? "Opening checkout…" : `Get Draft Pro — $${(passInfo.priceCents / 100).toFixed(2)} one-time`}</button> : <span className={styles.notice}>{checkoutUnavailableReason(checkoutAvailability.reason)}</span>}<span className={styles.notice}>One-time access through {formatEasternDate(passInfo.expiresAt)} (Eastern). No automatic renewal.</span></div><form className={styles.accessCodeForm} onSubmit={redeemAccessCode} aria-labelledby="draft-pro-access-code-title"><h3 id="draft-pro-access-code-title">Have an access code?</h3><p>Redeem a complimentary Draft Pro code for this signed-in account.</p><label htmlFor="draft-pro-access-code">Draft Pro access code</label><div className={styles.accessCodeRow}><input id="draft-pro-access-code" name="code" type="text" autoComplete="off" inputMode="text" minLength={20} maxLength={200} value={accessCode} onChange={(event) => setAccessCode(event.target.value)} disabled={accessCodeState === "submitting"} /><button type="submit" disabled={accessCodeState === "submitting"}>{accessCodeState === "submitting" ? "Redeeming…" : "Redeem code"}</button></div>{accessCodeState === "error" && accessCodeMessage ? <p className={styles.errorNotice} role="alert">{accessCodeMessage}</p> : null}</form></> : null}
