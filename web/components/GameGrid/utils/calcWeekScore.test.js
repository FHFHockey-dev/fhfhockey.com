...
describe("Carolina Hurricanes week score | startDate=2022-05-23&endDate=2022-05-29", () => {
  ...
  test("test week score", () => {
    // No toggle
    expect(calcWeekScore(gameScores, 3, 9, 3)).toBeCloseTo(19.64675, 5);

    // Turn off a day (e.g., Tue)
    const adjustedGameScores = [null, null, null, 0.53, null, 0.53, null];
    expect(calcWeekScore(adjustedGameScores, 2, 7, 2)).toBeCloseTo(12.95925, 5);

    // Turn off all days
    expect(calcWeekScore([], 0, 0, 0)).toBeCloseTo(-100);
  });
});
