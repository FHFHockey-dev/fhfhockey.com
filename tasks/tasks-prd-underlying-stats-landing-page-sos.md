...
- [ ] 4.0 Tighten the `/underlying-stats` landing-page layout and copy so it aligns with the FHFH table-heavy data-page style system
  ...
  - [ ] 4.3 Adjust the header and legend copy in `web/pages/underlying-stats/index.tsx` so the page stays accurate, concise, and operational rather than editorial.
  - [ ] 4.4 Ensure the added `SoS` column still fits within the table layout on desktop and remains usable in the current mobile/tablet responsive behavior.
  - [ ] 4.5 Reuse shared tokens and panel patterns from `web/styles/vars.scss` and `web/styles/_panel.scss` instead of introducing new local one-off styles.
- [ ] 5.0 Verify rendered output, data correctness, and scope safety for the landing-page-only change set
  - [ ] 5.1 Add or update targeted unit tests for date loading, trend handling, and `SoS` computation.
  - [ ] 5.2 Run targeted data checks against the underlying sources to confirm the displayed summary cards, table values, badges, and derived metrics match the intended formulas.
  - [ ] 5.3 Perform a rendered verification pass on the live local `/underlying-stats` route for desktop and mobile layouts after the changes.
  - [ ] 5.4 Confirm that no implementation changes altered `/underlying-stats/playerStats` behavior or shared logic beyond what was strictly required for the landing page.
  - [ ] 5.5 Record final verification notes, assumptions, and any residual risks around optional `SoS` inputs that were intentionally excluded.
