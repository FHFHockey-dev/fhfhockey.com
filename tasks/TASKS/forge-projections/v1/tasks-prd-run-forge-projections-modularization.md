...
- `tasks/TASKS/forge-projections/docs/FORGE_EXPLAINED.md` - Update stale source-file references to new runner filename.
- `tasks/TASKS/forge-projections/docs/FORGE_ECOSYSTEM_ELI5_AUDIT.md` - Update stale references to runner filename and architecture description.
- `tasks/TASKS/forge-projections/v1/prd/prd-run-forge-projections-modularization.md` - Source PRD and scope guardrails.
- `tasks/TASKS/forge-projections/v1/tasks-prd-run-forge-projections-modularization.md` - Active execution checklist, dependency tracking, and frozen reference inventory.
...
- [x] 6.0 Phase 6 - Reference Hygiene And Post-Parity Follow-Up Separation
  - [x] 6.1 Update internal docs/tasks/script references from `runProjectionV2.ts` to `run-forge-projections.ts` where they describe source file location (excluding intentional historical mentions). [Deps: 5.4] [Files: `tasks/TASKS/forge-projections/docs/FORGE_EXPLAINED.md`, `tasks/TASKS/forge-projections/docs/FORGE_ECOSYSTEM_ELI5_AUDIT.md`, `tasks/*.md`, `fix_terminal.sh`] [AC: no misleading active-path references remain]
...
- [ ] NEW 8.0 **P2 unfinished stage-oriented decomposition:** `run-forge-projections.ts` remains a mixed-responsibility orchestrator. Local progress (2026-07-29): concrete `stages/preflight-stage.ts` now owns season/game/market/roster-event context loading and `stages/metrics-finalization-stage.ts` owns final row/learning/pool/timeout metrics; the pass-through preflight, metrics, and lifecycle-persistence wrappers are removed, reducing the runner from 4,893 to 4,700 lines. Keep open until the per-game skater, goalie, and persistence responsibilities are extracted under the existing no-math-change parity guardrail.
