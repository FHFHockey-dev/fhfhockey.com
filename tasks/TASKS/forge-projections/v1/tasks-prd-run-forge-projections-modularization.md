...
- [ ] NEW 8.0 **P2 unfinished stage-oriented decomposition:** `run-forge-projections.ts` remains a mixed-responsibility orchestrator. Local progress (2026-07-29): concrete preflight, metrics-finalization, and persistence stages now own context loading, final metrics, all projection-table upserts, and ordered analytics-output replacement; all five original pass-through wrappers are gone and the runner falls from 4,893 to 4,643 lines with no direct mutation calls. Keep open until the per-game skater and goalie responsibilities are extracted under the existing no-math-change parity guardrail.
