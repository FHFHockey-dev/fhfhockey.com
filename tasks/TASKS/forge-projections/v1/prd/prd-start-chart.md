...
>
> **Current reconciliation status (2026-08-26): Reopened.** The earlier 55/55 checklist proved the canonical-wrapper architecture but did not prove source-accurate historical selection, degraded-state disclosure, row-identity ranking, the completed responsive board, or the current model/data rollout gates. The 2026-08-26 tranche is tracked in the reconciled task list. Local code and contract work is complete; Production data repair, controlled provider proof, the 14-date rollout gate, sandbox-blocked browser execution, and publication remain open. This status authorizes no persistence, migration, backfill, model promotion, deployment, or other external mutation.
>
...
Ranking is the canonical one-game FORGE stat line scored with `fhfh-default-skater-v1`. Ownership and weekly games are informational only. Projection rows are identified by run, game, player, and horizon; deterministic ties use score, player, game, team, then row key. Historical schedule, run, ratings, CTPI, goalie, and ownership reads are bounded to the resolved slate date. Missing values remain unavailable rather than becoming zero. `matchup_grade` is explicitly presentation-only **DEF ease** (higher opponent 5v5 xGA/60 is easier); FORGE's model-owned opponent-defense edge remains separately exposed in row context.

The Starter Board now exposes requested/resolved date, scoring/model/input/run provenance, source/coverage status, explicit degraded states, the five-position desktop board and responsive ARIA tabs, 25-row incremental disclosure, dated ownership behavior, normalized goalie mixtures, contextual cards, accessible CTPI labeling, and router-backed workflow links. `mode=week` is link context only and does not alter Daily ranking semantics.

The current local exact read for `2026-02-05` resolved run `44501906-53bd-48fc-9900-cf50c9845840`, 7 games, 14 teams, and 138 rows in 154,197 bytes. All 138 skater rows reconciled to the scoring contract within `0.001` (maximum rounding delta `0.0005`) and had zero rank mismatches. The same read honestly reports legacy/missing projection provenance and 0/14 goalie-team coverage; it is therefore degraded evidence, not a trusted rollout receipt. The `2026-02-07` no-game request deterministically fell back to `2026-02-05` and returned 154,338 bytes. A 20-read local cold-plus-warm sample measured 9.2 ms P95, with a separately disclosed 1,257.3 ms first uncached read.

Release remains gated by the unchecked 2026-08-26 tranche: authorized historical repair/application and provenance proof, controlled Yahoo provider/league equivalence where required, at least 14 distinct eligible holdout dates through the model rollout gate, successful Playwright runtime execution in an environment that permits Chromium, and an explicitly authorized publication/deployment. Quarantined legacy runs remain immutable and ineligible for trusted evaluation.
