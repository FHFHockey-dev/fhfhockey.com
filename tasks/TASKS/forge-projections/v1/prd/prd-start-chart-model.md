...
>
> **2026-07-27 checkpoint:** Start Chart remains a read-only wrapper over the latest succeeded canonical FORGE run. The supported control surface is one date, points mode, `fhfh-default-skater-v1`, optional position, and bounded pagination; tau/risk/alternate-model controls remain explicitly unavailable. Versioned server-owned competition ranks cover each eligible position; shared pure math now includes finite-safe rate/TOI Poisson means and evidence-gated negative-binomial selection; and the canonical coordinator exposes bounded/durable retry plus audited `200`/`207`, counts, freshness, and failure receipts. The current source list is 46/55, with nine independent formula/input-integration, history, manual-override, and exact Production-publication rows open.
