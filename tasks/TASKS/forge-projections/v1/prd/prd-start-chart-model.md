...
>
> **2026-07-27 checkpoint:** Start Chart remains a read-only wrapper over the latest succeeded canonical FORGE run. The supported control surface is one date, points mode, `fhfh-default-skater-v1`, optional position, and bounded pagination; tau/risk/alternate-model controls remain explicitly unavailable. Versioned server-owned competition ranks cover each eligible position, with canonical synthetic projection coverage retained in FORGE. The shared pure-math contract now has deterministic coverage for decay/effective sample size, weighted dispersion, slope, shrinkage, clipping, and goalie finishing. The current source list is 43/55, with 12 independent distribution/formula, history, manual override, retry, and exact Production-publication rows open.
