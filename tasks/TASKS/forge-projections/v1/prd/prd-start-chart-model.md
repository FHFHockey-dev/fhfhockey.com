...
>
> **2026-07-27 checkpoint:** Start Chart remains a read-only wrapper over the latest succeeded canonical FORGE run. The supported control surface is one date, points mode, `fhfh-default-skater-v1`, optional position, and bounded pagination; tau/risk/alternate-model controls remain explicitly unavailable. Versioned server-owned competition ranks cover each eligible position, the shared pure-math contract is deterministic, and the canonical coordinator exposes bounded/durable retry plus audited `200`/`207`, counts, freshness, and failure receipts. The current source list is 44/55, with 11 independent distribution/formula, history, manual override, and exact Production-publication rows open.
