...
>
> **2026-07-30 checkpoint:** Start Chart remains a read-only wrapper over the latest succeeded canonical FORGE run. The supported control surface is one date, points mode, `fhfh-default-skater-v1`, optional position, and bounded pagination; tau/risk/alternate-model controls remain explicitly unavailable. Versioned server-owned competition ranks cover each eligible position; shared pure math owns finite-safe rate/TOI Poisson means and evidence-gated negative-binomial selection; and the canonical coordinator exposes bounded/durable retry plus audited `200`/`207`, counts, freshness, and failure receipts. Guarded recovery `96ccea804` is READY/Production as `dpl_HCFwiK4yAPeXUG3QzC3R28NtYvsc`; populated API and exact desktop/mobile browser proof pass without runtime error or overflow. The owner removed the obsolete manual-override requirement, leaving the source list at 49/55 with six formula/input-integration and history rows open.
