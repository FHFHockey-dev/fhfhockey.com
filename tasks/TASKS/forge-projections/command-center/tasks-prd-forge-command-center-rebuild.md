...
- [ ] NEW 17.0 **P2 one-release page-usage evidence and final route decision.** Runtime evidence confirms active dependency traffic and 40 successful Sustainability responses; current Production also renders named Top Trust and Regression Risk rows, closing the separate A-SUST identity gate. Static `/forge/command-center` pageviews are not represented in runtime logs and neither available browser session is authenticated to Vercel Web Analytics. Preserve coexistence and `/forge/dashboard` rollback until the owner reviews the page evidence and explicitly selects continued coexistence or the final route swap.
- [ ] NEW 18.0 **P2 skater-power latency exceeds the recorded response target after timeout repair.** The six concurrent cold Production requests completed in 6.82–7.09 seconds; Vercel runtime evidence classifies the 1.99-second warmed probe as a static cache HIT, so that external wall time did not execute the function and is not valid function-p95 evidence. The local route now loads its four independent metric categories concurrently and emits Vercel-only value-free `metricFetchMs`/total-duration telemetry by cache status; the concurrency regression plus Command Center cohort passes 3 files/15 tests, TypeScript, scoped lint, and formatting. Keep open for guarded publication and a bounded representative structured-duration distribution against the unchanged 900 ms target.
