...
- [x] NEW 18.0 **P2 skater-power latency exceeds the recorded response target after timeout repair.** The local route loads its four independent metric categories concurrently and emits Vercel-only value-free `metricFetchMs`/total-duration telemetry by cache status. The selective rolling-column projection and cast-only `unknown` bridge are included in Production deployment `dpl_vU1c1yvbDQJMXEzK1rLT373Qndwy`; focused tests pass 10/10 with TypeScript, scoped lint, formatting, and the local production build. Five sequential varied cache-miss GETs returned 200 with structured `metricFetchMs` values of 113–173 ms and `durationMs` values of 203–386 ms (median 249 ms; every sample below the unchanged 900 ms target). A separate concurrent fanout produced one 1,146 ms outlier and is retained as a bounded contention observation rather than substituted for the sequential request cohort; cache-hit wall time is not treated as function-p95 evidence. The exact Production deployment has no bounded runtime-error cluster (closed 2026-07-31).
