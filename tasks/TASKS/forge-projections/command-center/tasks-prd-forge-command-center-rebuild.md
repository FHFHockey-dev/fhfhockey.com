...
- [x] 8.0 Add tests and source-to-UI reconciliation coverage
  ...
  - [x] 8.7 Record any unresolved data trust, stale-state, or route-continuity gaps in the reconciliation checklist before promotion. The selected-date leakage and route-hydration defects were repaired; the sustainability null-name source gap was tracked under A-SUST NEW 8.0 and is now closed by current Production route/render evidence.
...
- [x] NEW 16.0 **P1 live skater-power statement timeout under Command Center traffic.** Exact Production deployment `dpl_2DyH68oEiUhfsakGfRcHMAFdmumA` includes the index-aligned `game_date DESC, player_id` pagination repair. Six concurrent bounded historical scopes and one warmed repeat returned 200, the exact-deployment logs show six misses plus one cache hit with no statement timeout/500, and the bounded route-error query is empty (closed 2026-07-30).
- [x] NEW 17.0 **P2 one-release page-usage evidence and final route decision.** The owner approved Command Center as canonical after the completed coexistence period and existing dependency/render evidence. `/forge/dashboard` now performs a non-permanent server redirect to `/forge/command-center`, preserving only compatible date/resolved-date/team/position/slate/mode query parameters; obsolete rollback links and labels are removed. Focused redirect, Command Center, and link tests pass locally (closed 2026-07-30).
- [ ] NEW 18.0 **P2 skater-power latency exceeds the recorded response target after timeout repair.** The local route loads its four independent metric categories concurrently and emits Vercel-only value-free `metricFetchMs`/total-duration telemetry by cache status. A local follow-up now projects only the rolling-average column required by windows 3/5/10 (or none for raw/20-game windows), preserving fallback calculations; the focused suite passes 10/10 with TypeScript, scoped lint, and formatting. The exact READY/Current Production deployment `dpl_9oJFoypqT7RRqVWJWkefLk6xcE61` returned 200 for five varied cold-cache GETs (wall time 0.832–3.324 seconds); its available structured serverless miss recorded `metricFetchMs=1385` and `durationMs=1644`, above the unchanged 900 ms target. The first Git-triggered build for pushed `e6a93bc23` failed at this route's direct Supabase-result cast; a cast-only `unknown` bridge is now locally verified (10/10, TypeScript, scoped lint, formatting, and the local production build), but remains unpublished. Keep open for a representative structured-duration distribution and a Production recheck of the fixed optimization; no cache-hit wall time is treated as function-p95 evidence (updated 2026-07-31).
