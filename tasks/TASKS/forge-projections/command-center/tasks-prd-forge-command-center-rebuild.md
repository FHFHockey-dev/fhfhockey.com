...
- [ ] NEW 16.0 **P1 live skater-power statement timeout under Command Center traffic.** Seven-day Vercel evidence contains 11 HTTP-200 and one HTTP-500 skater-power responses plus three clustered statement-timeout errors, most recently 2026-07-29 on `shots_per_60`. The local forward repair changes deterministic pagination from full-sort `player_id, game_date` ordering to index-aligned `game_date DESC, player_id`; production `EXPLAIN` drops first-page startup cost from `2154.89` to `60.26` through the existing metric/date index and incremental sort, and the direct plus Command Center cohorts pass 14/14. Keep open for guarded publication/deployment and bounded live no-timeout verification.
