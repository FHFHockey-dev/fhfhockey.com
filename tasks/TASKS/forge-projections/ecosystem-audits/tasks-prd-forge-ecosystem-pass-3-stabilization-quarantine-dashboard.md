...
- The audit evidence includes the legacy schedule entries in `tasks/TASKS/cron-operations/cron-schedule.md` plus benchmark and normalized-inventory artifacts that still name `update-rolling-games-recent` and `update-power-rankings` as scheduled HTTP jobs.
...
- [x] 7.0 Clean up stale documentation and task references that still describe `runProjectionV2.ts` as an active projection-runner file
  - [x] 7.1 Update active docs and task guides that still describe `web/lib/projections/runProjectionV2.ts` as a live runtime path so the repo no longer teaches the removed shim as current architecture. [Deps: 2.3] [Files: `tasks/TASKS/forge-projections/docs/FORGE_EXPLAINED.md`, `tasks/*.md`, related active runbooks/docs] [AC: active guidance points to `run-forge-projections.ts` unless a historical note is explicitly marked as historical]
...
- `tasks/TASKS/cron-operations/cron-schedule.md` still carries disabled-route schedule snippets for:
...
- `tasks/TASKS/cron-operations/cron-schedule.md` now marks the legacy schedule snippets as `DISABLED IN PASS 3` historical entries.
...
- Updated `tasks/TASKS/forge-projections/docs/FORGE_EXPLAINED.md` so the refresh-order guidance now points to the canonical runner in `run-forge-projections.ts` instead of the removed `runProjectionV2` shim name.
...
- Validation for this sub-task:
- `rg -n "runProjectionV2\\.ts|runProjectionV2\\b" tasks/TASKS/forge-projections/docs/FORGE_EXPLAINED.md tasks web -g '!web/node_modules'`
...
- `tasks/TASKS/cron-operations/cron-schedule.md` still carries the `09:40 UTC` `update-start-chart-projections` schedule entry.
...
- `rg -n "update-start-chart-projections|player_projections" tasks web/rules tasks/TASKS/forge-projections/docs/FORGE_EXPLAINED.md -g '!web/node_modules'`
...
- `tasks/TASKS/cron-operations/cron-schedule.md` now marks the `09:40 UTC` slot as retired in pass 3 rather than listing a live cron definition.
...
- `tasks/TASKS/schema-docs/supabase-context/goalie-tables.md` now documents the pass-3 ownership decision beside the table definition.
