...
- `web/pages/trends/player/[playerId].tsx` - Canonical player diagnostic surface that accepts validated FORGE metric-group and metric-selection handoff state.
...
- `tasks/TASKS/rules/fhfh-styles.md` - Repository styling blueprint that informs dashboard polish decisions if no dedicated `styles/fhfh-styles.md` exists.
...
- `web/__tests__/pages/trends/player/[playerId].test.tsx` - Query-driven diagnostic handoff regression coverage.
...
- [x] 16.0 Repair dashboard styling-system drift and responsive layout defects uncovered during the pass-4 styling audit
  ...
  - [x] 16.3 Bring FORGE control-state styling back in line with `tasks/TASKS/rules/fhfh-styles.md` for compact dashboard actions, especially the active `TopAddsRail` mode toggle and related segmented controls that still read weaker than the repo’s canonical highlighted dashboard button treatment.
...
- [x] 20.0 Tighten reader freshness truthfulness where pass-4 smoke tests still exposed stale fallback behavior on live FORGE-facing routes
  ...
  - [x] 20.3 Either populate or explicitly retire the missing goalie uncertainty metadata surfaced during smoke tests, because `/api/v1/forge/goalies` currently reports missing model-version and starter-scenario-count notes in its scan summary. Current implementation extracts `model.scenario_metadata.model_version` and top-two/starter scenario counts, exposes stable response metadata, and retains explicit missing-metadata notes only for genuinely incomplete rows; the focused API contract passes 3/3 tests (2026-07-12).
...
- [x] 21.0 Repair output-level plausibility defects surfaced during the component vetting pass. Sustainability outliers, tiny-sample trend ranks, placeholder goalie calls, and mixed-recency team context now fail or degrade truthfully with focused regression coverage (2026-07-12).
  ...
  - [x] 21.2 Re-audit `web/pages/api/v1/trends/skater-power.ts` percentile and delta math for tiny-game samples, because pass-4 smoke data showed three-to-six game rows producing `100` percentile plus triple-digit deltas, which reads as edge-case-sensitive and hockey-misleading even before the stale-date issue. Sub-10-game percentiles now shrink linearly toward neutral, raw percentiles and sample confidence remain explicit, and rank deltas stay suppressed until both current/prior samples qualify; 5/5 focused API tests and TypeScript pass (2026-07-12).
  - [x] 21.3 Rework goalie fallback presentation so rows with blanket `0.50` starter probabilities and null recommendation/confidence fields do not surface as ordinary lead-card decisions in `GoalieRiskCard`. Placeholder rows remain visible only in the reference table with `Context unavailable`, are excluded from spotlight calls, and trigger an explicit warning; dashboard/normalizer regressions pass 31/31 (2026-07-12).
  - [x] 21.4 Add stronger mixed-recency invalidation or downgrade rules to `web/components/forge-dashboard/TeamPowerCard.tsx`, because current output composition can blend team ratings from `2026-02-07`, start-chart matchup context from `2026-02-05`, and CTPI generated on `2026-03-29` into one visible ranking surface. CTPI form and slate matchup inputs now join only when their resolved dates equal the team-ratings snapshot; mismatched values are suppressed with explicit source-date warnings, and 26/26 dashboard tests plus TypeScript pass (2026-07-12).

- [x] 22.0 Retire or isolate the remaining orphaned legacy sKO chart artifacts that survived the route quarantine. The no-consumer chart pair remains explicitly documented for lineage rather than deleted, while generic data contracts/exports are renamed to unmistakable `LegacySko*` names; TypeScript passes (2026-07-12).
  - [x] 22.1 Decide whether `web/components/GameScoreChart/GameScoreChart.tsx` and `web/components/GameScoreChart/GameLogTable.tsx` should be deleted, moved under a legacy namespace, or explicitly documented as quarantine-only artifacts now that `web/pages/skoCharts.tsx` no longer renders the old runtime. Chosen disposition: retain as explicit quarantine-only lineage artifacts because they have zero live consumers and deletion is unnecessary churn; file headers prohibit FORGE use (2026-07-12).
  - [x] 22.2 Decide whether the legacy `CombinedGameLog` / `fetchGameLogs` path under `web/lib/supabase/utils/` should be further narrowed, renamed, or moved so future FORGE work cannot mistake it for an approved shared data contract. Renamed to `LegacySkoCombinedGameLog`, `fetchLegacySkoGameLogs`, and `fetchLegacySkoMostRecentSeason`; all legacy component/calculation references use the explicit type, no generic alias remains, and TypeScript passes (2026-07-12).

- [x] 23.0 Strengthen the FORGE-to-Trends diagnostic handoff instead of creating a duplicate player-diagnostic route. Validated URL state now focuses the canonical Trends diagnostic surface, and FORGE source/date/return context remains intact; 29/29 focused tests and TypeScript pass (2026-07-12).
  - [x] 23.1 Add query-driven metric-group and metric-selection handoff support to `web/pages/trends/player/[playerId].tsx` so FORGE cards can open the player diagnostic page with the relevant signal family already focused. The page validates group-scoped metric keys, falls back safely, reflects active controls accessibly, and serializes later control changes through shallow routing (2026-07-12).
