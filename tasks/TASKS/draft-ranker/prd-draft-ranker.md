...
- Dynamic Wave-C review on 2026-07-29 closed NEW DR-084/085: the three protected maintenance routes now expose stable unavailable contracts instead of raw dependency details, and CSV exports neutralize formula-leading string cells without changing numeric values. Read-only Production catalog evidence confirms all 21 Draft Ranker tables have RLS, all 18 Draft Ranker functions are security-invoker and service-role-only, owner policies use `auth.uid()`, and private community/discovery tables remain client-denied. The pre-baseline migration chain remains represented by the authoritative archive plus the live `20260716112908` baseline; no schema or Production state changed.
- DR-080 through DR-083 and their Phase-8 parent are explicitly retired to the future roadmap; their scope is not part of the active super-goal.
