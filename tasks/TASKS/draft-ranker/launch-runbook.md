...
- current staff/homepage deployment: `dpl_6ZtPqJywoSjdC1iS3rhkYrSJDU2v`, READY from source commit `e5bd6c83876cb6bedc981fed3bf3e0ad4e67eded` and promoted preview `dpl_ArdcG4AwskbLTgBUYLVT6aGehohD`;
...
For a homepage-only incident, first disable `DRAFT_RANKER_HOMEPAGE_ENABLED`; this restores Transaction Trends while preserving the staff-only personal route. If the incident is broader, set the rollout stage `off` and disable the master flag. Deployment `dpl_JDS7NpzpHgJa56ppWGQYoAcN7n8H` is the immediate pre-hydration-fix artifact, `dpl_8RitVqdJ9QsRTi11J7xjH66u6cdu` is the pre-homepage artifact, and `dpl_HgjrBeCQVzDTsgryafBLzmXqGyjH` remains the older retained rollback reference. No other secondary flag or cohort expansion is permitted until the original staff evidence gate is completed, reviewed, and separately approved.
