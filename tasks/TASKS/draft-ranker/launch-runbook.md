...
- [ ] The paths in [release-manifest.md](./release-manifest.md) have been assembled from a clean current remote base into a reviewed READY production deployment that serves the feature routes with every rollout flag off.
...
A read-only Vercel preflight on 2026-07-15 found that the READY production deployment predates Draft Ranker and the live `/draft-rankings` route returns 404. Remote `octoberBranch` is currently `0ed825fb30f5c6f329cd0dc169d2d9b2ca975e0f`, a newer published sibling of the local checkpoint, and its newest deployment failed on the `ProcessEnv` typing issue in `rateLimit.ts`. That issue is corrected in the scoped candidate. [release-manifest.md](./release-manifest.md) classifies every dirty path and records a clean isolated assembly from the current remote base: 102 included paths, zero conflicts/collisions, 228/228 Draft Ranker tests, 54/54 broader WIGO tests, passing TypeScript, and a complete 83-page production build. The isolated branch remains local-only because staging, pushing, and opening a draft PR require explicit publication authorization. Production flags remain off until the reviewed manifest artifact is deployed READY and the named owners, exact cohort UUIDs, and observation-window record complete the remaining operational checklist.
