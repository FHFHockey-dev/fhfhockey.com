...
Status/Scope: Release infrastructure complete and product-owner staff-stage approval recorded. The live production deployment currently predates Draft Ranker, so a reviewed goal-scoped release artifact must first reach a READY production deployment with every flag off. Actual staff, allowlisted, authenticated, and public cohort observation windows remain pending named operators, exact cohorts, and real traffic.
