...
- Replay receipt (2026-07-31): the separately authorized idempotent call returned 851 upserts with the same writer digest, while the read-only post-check preserved 851 target rows and the unrelated 9,765-row count/digest exactly. The run manifest is `succeeded` at attempt 3, the latest cron-audit row is HTTP 200 with 851 affected rows and no error, and the bounded two-hour Production runtime-error query is empty. Replay and unrelated-scope invariance are therefore satisfied.
- Rollback/repopulation receipt (2026-07-31): after separate authorization, a guarded Supabase delete removed exactly 851 rows only when the retained target count and writer digest matched. The corrected natural offseason request then returned a successful zero-publication manifest at attempt 2 with `skipReason=outside_official_nhl_season`, HTTP 200 audit status, and zero affected rows. The owner re-populated the exact manifest; attempt 4 returned 851 upserts in five batches, `partial=false`, health `ok`, and the unchanged writer digest.
- Final acceptance (2026-07-31): the post-repopulation read-only check found 851 target rows with digest `7a3198ade1f0edc8498291f83127a546100912c39611f4610309cdd8d9c54468` and 9,765 unrelated rows with the frozen JSONB-row digest `36e70852795ed519604390f718158c45f7061694fa47b2fc06cd8af86919d3e1`. Target and offseason manifests are `succeeded` at attempts 4 and 2; the latest population audit is HTTP 200 with 851 affected rows, the natural audit is HTTP 200 with zero affected rows, and the bounded two-hour Production runtime-error query is empty. The one-off now satisfies the seasonal writer's temporary Production evidence gate until opening day; it remains historical compatibility evidence, not a scheduled run or model promotion.
...
- The accepted sKO cohort has its exact guarded rollback/repopulation receipt above. Any future stage owner must define atomic outputs, run identity, idempotency, partial-failure recovery, and rollback before first execution.
