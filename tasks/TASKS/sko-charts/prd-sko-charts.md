...
>
> **Runtime status (2026-07-23):** This is the governing promotion/requirements PRD, not proof of current production formula ownership. Current supported Trends uses rolling/FORGE data; legacy GameScore/characteristic helpers and `predictions_sko` remain quarantined, the active compatibility writer persists a separate moving-average v0.2 score, and the documented local modeling scripts were deleted. Both privileged Next.js writers are now fail-closed in customer Production: exact deployment `dpl_4q8GM1wqTbHR3NbnxRxAakdLEvJr` passes value-free 401/401/405 proof on each route with no writer or bounded runtime error, closing NEW 9.1/9.5. The exact Production Python functions deployment still returns 404 for its separate miswired route; no executable model stage exists and NEW 9.3 retains restore/replace/retire ownership. See `sko-ownership-contract.md`, `sko-runbook.md`, and NEW 9.2–9.15.
