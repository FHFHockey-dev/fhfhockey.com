...
**2026-07-31 continuous-discovery closure:** The owning sKO reader/writer suites pass 25/25 tests, TypeScript and zero-error lint pass, and diff integrity passes. NEW 9.1–9.17 contain every verified defect, manual/provider dependency, model question, and optimization found to date; the re-read found no new item outside the explicit Production/provider/history/retirement gates. B-SKO is complete at 70/70; no runtime or external state changed.

> **Reconciled implementation task list:** `tasks/TASKS/sko-charts/tasks-prd-sko-charts.md`
>
> **Runtime status (2026-07-31):** This is a historical promotion/requirements PRD, not proof of current production formula ownership. The owner authorized the deleted modeling family and orphaned prediction UI as historical-only quarantine. Current supported Trends uses rolling/FORGE data; legacy GameScore/characteristic helpers and `predictions_sko` remain non-authoritative quarantine, and the active moving-average v0.2 writer remains compatibility-only pending separate retirement gates. Both privileged Next.js writers are fail-closed in customer Production at deployment `dpl_4q8GM1wqTbHR3NbnxRxAakdLEvJr`. Source date/window responses now expose freshness, every source-family count, family-specific missing-player associations, and atomic attempted/upserted/partial state; prediction manifests retain matching coverage/source/write health. The bounded reader returns deterministic compatibility, unavailable-feature, stale-snapshot, and empty-data warnings alongside its complete nullable identity/prediction/CV/model/timestamp contract. Ordered Production migrations `20260728225806` and `20260729205048` are applied and verified; they preserve per-model compatibility history and narrow public table grants without promoting the score. Fresh free local execution proves run-control lease behavior and rollback. During the offseason, an accepted exact-manifest historical in-season population may satisfy the seasonal writer's Production evidence gate until opening day, while the natural no-write receipt separately proves scheduler policy. No executable model stage exists, no score is promoted or reinterpreted, and the plan below is not authorization to restore it. See `sko-ownership-contract.md`, `sko-runbook.md`, and NEW 9.2–9.17.
...
- Ops
  ...
  - [x] Alerting if failure or unusually low updated rows. *(Stable health codes classify failed/partial writes as errors and selected-versus-written shortfalls as warnings; the verdict and exact diagnostics are returned, persisted in the run manifest, emitted to runtime logs, and surfaced by the existing daily cron operator email. The 2026-07-31 value-free reader observation found only the three expected compatibility warnings and no runtime-error, schema-drift, artifact-mismatch, or stalled-stage signal.)*
...
Ordered Production migrations `20260728225806` and `20260729205048` are applied before exact-SHA application publication. Value-free catalog proof finds all three sKO run-control RPCs security-invoker, browser-denied, and service-only, with model-version prediction identity preserved. This closes persistence parent 2.0 and migration row 2.2; B-SKO is 69/70 after the accepted one-off, monitoring observation, documentation synchronization, and 8.0 parent closure. Only continuous discovery NEW 9.0 remains open. The accepted exact-manifest compatibility population, guarded rollback/repopulation, natural offseason no-write receipts, and 2026-07-31 reader schema/freshness/coverage/runtime observation satisfy the temporary offseason and monitoring evidence gates.
