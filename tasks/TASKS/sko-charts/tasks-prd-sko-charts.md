...
- [ ] NEW 9.0 Append every verified defect, manual/provider dependency, model question, and optimization discovered during execution here before closure.
  ...
  - [ ] NEW 9.15 **P2 raw dependency error disclosure from the public prediction reader:** `/api/v1/ml/get-predictions-sko` returns the database client's raw error message in its public 500 response and logs the unsanitized value. Replace the public body with a stable operator-safe error contract, retain only sanitized server diagnostics, and add empty/error-state regression evidence before closing 5.4/5.5 (discovered 2026-07-22).
