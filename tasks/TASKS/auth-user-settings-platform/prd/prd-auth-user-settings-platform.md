...
- Production and localhost Google OAuth are verified: localhost `http://localhost:3000/auth` completed the hosted Supabase callback and returned to a credential-free authenticated `/auth` state, followed by sign-out and local auth reset. Custom SMTP configuration, Supabase confirmation/recovery handoff, and recipient-side Outlook Inbox receipt for both exact messages are verified; received-link confirmation/recovery completion, password update, and cleanup remain open. Preview auth is explicitly unsupported. The JavaScript browser client now explicitly uses PKCE; hosted template compatibility and complete lifecycle proof remain open under `NEW 46.0`.
...
- The current client explicitly uses PKCE. Compatible confirmation/recovery templates plus complete Google, confirmation, and recovery verification remain required under `NEW 46.0` before legacy fragment compatibility is retired.
