...
**2026-08-01 hosted PKCE template/Production lifecycle checkpoint:** Supabase Confirm signup and Reset Password templates now use `{{ .RedirectTo }}` plus `{{ .TokenHash }}` links. A disposable Production sign-up delivered its confirmation email and value-free server readback showed one confirmed and signed-in user, but the app callback ended on a generic client navigation failure; the recovery link reached `/auth/reset-password`, password update succeeded, sign-out completed, and cleanup left zero disposable auth/profile/settings rows. The local callback now treats post-auth router rejection as a non-auth failure with a credential-free hard-navigation fallback. NEW 46.0 remains open until this fix is guardedly published and fresh value-free Production confirmation/Google/recovery evidence passes; no message values or tokens were recorded.

**2026-08-01 current disposable email lifecycle receipt:** A fresh disposable sign-up confirmation and password-recovery message were delivered through the active Supabase SMTP configuration. Confirmation returned through the hosted Supabase callback to a credential-free authenticated local `/auth` state; recovery rendered `/auth/reset-password`, password update succeeded, and disposable auth/profile/settings cleanup verified zero remaining rows. No message values or tokens were recorded. Focused AuthForm/callback/reset/location/client tests now pass 5 files/35 tests. Hosted `{{ .RedirectTo }}` template confirmation and value-free Production lifecycle publication remain under NEW 46; preview auth remains unsupported.

**2026-08-01 current PKCE implementation receipt:** The browser Supabase client explicitly selects `flowType: "pkce"`; callback and reset routes exchange authorization codes, preserve token-hash compatibility, scrub credential-bearing locations synchronously, retain safe same-origin return paths, and fall back to credential-free hard navigation when post-auth router replacement rejects. The focused AuthForm/callback/reset/location/client group passes 5 files and 35/35 tests, including a direct PKCE configuration regression. Hosted `{{ .RedirectTo }}`/`{{ .TokenHash }}` templates are configured; guarded publication and complete value-free Production lifecycle evidence remain open after a server-confirmed confirmation whose callback UI showed a generic navigation failure. The disposable local confirmation/recovery/password-update/cleanup lifecycle is complete and preview auth remains unsupported.

**2026-08-01 authorized Production PKCE verification:** Exact commit `4c10509c1` is published on `octoberBranch`; Production promotion `dpl_47AFWu6itJvqzrpeWkd1fTuQkVjq` is READY on the canonical aliases. Value-free route reads for `/auth`, `/auth/callback`, and `/auth/reset-password` returned `200`, with no bounded Auth runtime-error cluster. A fresh disposable confirmation now returns to clean `/auth` without the prior callback-navigation failure; server readback observed one confirmed user plus one profile/settings row, followed by exact zero-residual cleanup. A fresh recovery link rendered `/auth/reset-password`, password update/sign-out succeeded, and cleanup remained zero-residual. Current Google verification reached the expected Google OAuth identifier page without recording callback values; full provider completion remains deferred under the explicit no-provider boundary, so legacy fragment compatibility and NEW 46.0 stay open.
...
- [AuthProviderContext](/Users/tim/Code/fhfhockey.com/web/contexts/AuthProviderContext/index.tsx) now provides global session-first auth state from [\_app.tsx](/Users/tim/Code/fhfhockey.com/web/pages/_app.tsx), enriches optional roles without blocking the user shell, centralizes fail-closed sign-out, and exposes targeted local FHFH auth reset behavior to every route/header surface.
...
- Patreon now has an account-settings-only OAuth v2 flow using `identity` scope, explicit member/tier fields for the configured FHFH campaign, stable-ID anti-sharing, Vault-backed rotating tokens, provider sync runs, generic `patreon_supporter` eligibility, and cascade-backed disconnect cleanup. The bounded production connect/resync/duplicate-identity/disconnect lifecycle is verified; positive active-patron evidence remains separately open.
...
- Production and localhost Google OAuth are verified: localhost `http://localhost:3000/auth` completed the hosted Supabase callback and returned to a credential-free authenticated `/auth` state, followed by sign-out and local auth reset. Custom SMTP configuration, Supabase confirmation/recovery handoff, recipient delivery, and a disposable link/callback/reset/password-update/cleanup lifecycle are verified. Preview auth is explicitly unsupported. The JavaScript browser client now explicitly uses PKCE; hosted template compatibility and complete Production lifecycle proof remain open under `NEW 46.0`.
...
- A bounded Sent Items verification on 2026-07-21 unexpectedly exposed older plaintext credential-bearing messages unrelated to the requested support thread. Values must never be repeated, copied, tested, or persisted. P1 `NEW 51.0` keeps the already-rotated FHFH password family deduplicated under NEW 48 while requiring value-free ownership/status identification, retirement or rotation of any other affected credential, and explicit owner approval before destructive mailbox cleanup. A subsequent metadata-only inventory bounded the candidate scope to one 2025 `FHFH` subject and three 2022 `fhfhockey.com` developer-account subjects sent to a Gmail-domain recipient, without reading bodies, values, IDs, addresses, or links. Exact system ownership and retirement state remain owner-supplied redacted facts rather than safe mailbox inferences.
...
- The dedicated `/auth/callback` route accepts code, token-hash, and fragment-session representations for OAuth/verification completion, sanitizes same-origin returns, and synchronously scrubs credential-bearing visible/Next-history state before asynchronous auth work.
- Forgot-password targets `/auth/reset-password` directly. The reset route accepts code, recovery token-hash, or fragment-session payloads and updates the password through a bounded authenticated `/auth/v1/user` request rather than the previously hanging `supabase.auth.updateUser` call.
- The singleton browser client stores/refreshes supported Supabase sessions but disables automatic URL payload detection because the two dedicated routes process and scrub those payloads explicitly.
- The current client explicitly uses PKCE. Compatible confirmation/recovery templates plus complete Production Google, confirmation, and recovery verification remain required under `NEW 46.0` before legacy fragment compatibility is retired; the disposable local lifecycle is already complete.
- The global auth provider publishes the authenticated session user before optional role enrichment so the header/account shell initializes without route-specific wrapping or a secondary-role-query stall.
...
- The Patreon foundation rejects duplicate stable identities, reports configured-campaign membership/tier/status state, controls manual re-sync attempts, and has completed a bounded production disconnect with no retained target entitlement, token, sync, or referenced Vault-secret rows. Positive active-patron evidence remains separately gated.
