...
- `tasks/TASKS/nhl-game-prediction-model/prd-nhl-game-prediction-model.md` - Source PRD for scope, schema direction, phases, and acceptance criteria.
- `tasks/TASKS/nhl-game-prediction-model/research/deep-research-report.md` - Research basis for feature families, validation approach, calibration, and operational guardrails.
- `tasks/TASKS/schema-docs/supabase-context/supabase-table-structure.md` - Existing Supabase table reference for core identity, WGO, NST, standings, games, and player tables.
- `tasks/TASKS/schema-docs/supabase-context/supabase-views.md` - Existing view reference, including unified player/goalie views and latest-only view caveats.
- `tasks/TASKS/three-pillars-analytics/underlying-stats/power-ratings-tables.md` - Existing `team_power_ratings_daily` definition and rating math to audit before model use.
- `tasks/TASKS/forge-projections/context/forge-tables.md` - Existing FORGE runs, projections, game-strength, and roster-event schema reference.
- `tasks/TASKS/schema-docs/supabase-context/goalie-tables.md` - Existing goalie stats and `goalie_start_projections` schema reference.
- `tasks/TASKS/schema-docs/supabase-context/nst-team-tables-schemas.md` - NST team source table contracts for xG, shot, chance, and special-teams features.
- `tasks/TASKS/schema-docs/supabase-context/nst-goalie-table-schemas.md` - NST goalie source table contracts for goalie-quality features.
- `tasks/TASKS/schema-docs/supabase-context/player-table-schemas.md` - Player/skater source table contracts for optional lineup/player context.
...
- [x] 12.0 NEW: Restore playoff postgame scoring coverage without weakening immutable history or regular-season outcome precedence
  ...
  - [x] 12.3 Re-run v6 playoff scoring read-only against stored production data. The original row-level receipt found 53 stored predictions and evaluated 31 prediction rows at 61.2903% accuracy, 0.225645 Brier, 0.643972 log loss, and 0.638655 AUC with `dryRun=true` (2026-07-12). A 2026-08-28 identity audit corrects the denominator: those rows cover 13 unique games and only seven distinct completed games. Latest-pregame-per-game historical metrics are 4/7, 0.227105 Brier, 0.646903 log loss, and 0.583333 AUC; repeated horizons may not be presented as independent completed games.
