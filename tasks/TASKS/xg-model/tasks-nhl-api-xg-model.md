...
- [x] 3.0 Finalize the scalable schema and migration plan
  - [x] 3.1 Expand `tasks/TASKS/xg-model/nhl-api-foundation/schema-recommendation.md` to explain why raw snapshots plus normalized event and shift rows are preferred over a primary per-game JSONB design.
...
- [x] 7.0 Validate, backfill, and prepare for rollout
  - [x] 7.1 Create `tasks/TASKS/xg-model/nhl-api-foundation/validation-checklist.md` covering raw payload parity, normalized event counts, shot-family counts, score progression sanity, strength-state sanity, and shift overlap sanity.
