...
- `web/lib/xg/hardeningAudit.ts` - Segment calibration, internal benchmark-universe, and external taxonomy-alignment contracts for Deep Research hardening.
- `web/lib/xg/hardeningAudit.test.ts` - Regression coverage for segment insufficiency/metrics, benchmark filters, and provider-taxonomy safeguards.
- `web/lib/xg/flurryAdjusted.ts` - Retains raw shot xG while deriving a parallel flurry-adjusted probability layer.
- `web/lib/xg/aggregates.ts` and `aggregates.test.ts` - Persist raw and flurry-adjusted team/player/goalie game and rolling values with fail-closed metadata coverage.
- `web/pages/api/v1/db/update-nhl-xg-aggregates.ts` - Paginated aggregate writer that attaches exact-version flurry metadata before dry-run or upsert.
- `supabase/migrations/20260712234009_add_xg_flurry_adjusted_aggregate_columns.sql` - CLI-generated, unapplied nullable parallel-column migration for raw-preserving flurry materialization.
- `web/lib/xg/residualSkill.ts` - Builds chronological shooter-finishing and goalie-save residual layers without leaking future outcomes.
- `web/lib/xg/reboundHeads.ts` - Defines separate rebound creation, danger, goalie-control, and second-chance output heads with availability state.
- `web/lib/xg/trackingAlignment.ts` - Fail-closed event/frame/coordinate/identity/puck tracking eligibility audit.
- `web/lib/xg/operationsHealth.ts` - Builds operations alerts and sampled team aggregate reconciliation evidence.
- `web/pages/api/v1/xg/operations.ts` - Admin-only, read-only xG operations telemetry API with bounded/paginated source reads.
- `web/pages/underlying-stats/xg/operations.tsx` - xG operations dashboard with loading, authorization, partial, warning, and unavailable states.
- `web/pages/underlying-stats/xg/operations.module.scss` - Responsive operations dashboard styles.
- `web/__tests__/pages/underlying-stats/xg/operations.test.tsx` - Rendering coverage for partial telemetry and authorization failure.
- `web/lib/xg/executionLease.ts` - Owner-token database lease wrapper for long-running Pages API jobs.
- `web/lib/xg/executionLease.test.ts` - Lease conflict, completion, and failure-path regression tests.
- `web/scripts/audit-xg-registry-feature-coverage.ts` - Read-only paginated reconstruction/audit for legacy registry coverage metadata against immutable dataset row IDs.
- `supabase/migrations/20260712214739_xg_execution_leases.sql` - Applied owner-safe xG execution lease schema and service-role RPC migration.
- `supabase/migrations/20260712230000_fix_xg_execution_lease_acquire_conflict.sql` - Applied corrective migration using the named primary-key conflict constraint.
...
- `web/scripts/train-nhl-xg-baseline.ts` - Current shot-goal xG training path plus SHA-256-verified frozen dataset export for offline challengers.
- `web/scripts/train_nhl_xg_challengers.py` - Offline-only calibrated LightGBM/CatBoost/XGBoost and strength-state-partitioned challenger evaluator.
- `web/scripts/requirements.txt` - Python analysis/model dependencies, including LightGBM, XGBoost, and CatBoost challenger families.
...
- [x] 23.0 Apply Deep Research xG hardening recommendations
  ...
  - [x] 23.6 Add rink/arena, playoff, strength-state, and score-state calibration audits. Typed audits report sample/event counts, goal rate, average prediction, Brier, log loss, ECE, maximum calibration gap, and explicit insufficient/single-class warnings (2026-07-12).
  - [x] 23.7 Build separate internal benchmark surfaces for all-situations unblocked xG and 5v5 non-empty-net unblocked xG. Definitions preserve event, strength, empty-net, and score-state filters alongside metrics (2026-07-12).
  - [x] 23.8 Investigate NST/MoneyPuck/Evolving-Hockey taxonomy deltas before treating external xG disagreement as model failure. MoneyPuck/Evolving-Hockey primary methodology is recorded; NST remains provider-verification-required because current public methodology was inaccessible. Every comparison is fail-closed on taxonomy alignment (2026-07-12).
  - [x] 23.9 Train calibrated challenger models under the same frozen contract: LightGBM, CatBoost, XGBoost, and state-partitioned variants. Offline-only global plus EV/PP/SH/EN runs align all 118,299 frozen rows and validation-only Platt calibration; honest corrected test AUC is 0.812505/0.813102/0.813338 and Brier is 0.059168/0.059299/0.059298 for LightGBM/CatBoost/XGBoost, with no promotion path (2026-07-12).
  - [x] 23.10 Add flurry-adjusted xG as a parallel aggregate surface while retaining raw xG. Production now exposes complete parallel adjusted columns on all six active-model aggregate surfaces while preserving byte-stable raw fingerprints (2026-07-12).
  - [x] 23.11 Separate baseline shot quality from lagged shooter-finishing and goalie-save residual skill layers. Chronological/as-of-only lagged layers use minimum-sample shrinkage and remain separate from baseline xG; the operations surface reports shooter/goalie layer availability (2026-07-12).
  - [x] 23.12 Expand rebound modeling into separate rebound creation, rebound danger, goalie freeze/control, and second-chance xG heads. `rebound_rich_labels_v1` and its three frozen-split offline logistic heads are complete, separate, promotion-disabled, and absent from production registry/prediction storage (2026-07-12).
  - [x] 23.13 Continue controlled NHL replay/tracking audits and use real tracking only after stable event-to-frame alignment is proven. The bounded NHL samples remain goal-only (13/13 replay events across the two-game smoke, zero non-goal events); normalized-frame QA explicitly marks goal-only evidence ineligible for training, so no tracking feature was enabled (2026-07-12).
  - [x] 23.14 Add database-backed advisory locks or queue-backed execution for long-running xG backfills, prediction scoring, and aggregate builds. Production now uses owner-token database leases for feature, prediction, and aggregate writers; overlap rejection, owner-only mutation, stale takeover, and deployed runtime behavior are verified (2026-07-12).
  - [x] 23.15 Add an xG operations dashboard for feature null-rate drift, prediction coverage, calibration drift, aggregate reconciliation, and artifact-contract mismatches. Admin-only API/UI now exposes explicit partial/unavailable states, sampled 5v5/calibration/derived metrics, exact coverage counts, registry parity, lease state, and sampled xGF/xGA symmetry (2026-07-12).
  ...
  - Added `/api/v1/db/register-nhl-xg-model`. It loads `modelArtifactPath` or `NHL_XG_MODEL_ARTIFACT_PATH`, verifies the artifact feature contract, and upserts the registry row. Optional query params: `deploymentAlias`, `active=true`, `champion=true`, `confirmPromotion=true`, and `approvalStatus=candidate|approved|rejected|retired`; activation or champion promotion requires approval eligibility, approved status, explicit confirmation, and an active champion.
  ...
  - Added `web/lib/xg/hardeningAudit.ts` for four-axis calibration audits and explicit all-situations-unblocked versus 5v5/non-empty-net/unblocked benchmark universes. Provider taxonomy rows record source URLs/access dates and never classify disagreement as failure before event/strength/rink/flurry/rebound/empty-net/calibration alignment.
  - MoneyPuck primary methodology confirms unblocked-shot xG, arena-adjusted coordinates, explicit empty-net context, raw/flurry-adjusted variants, and separate rebound/freeze/created-xG concepts. Evolving-Hockey primary methodology confirms a Fenwick binary shot-goal target but leaves several selected-surface details requiring verification. Natural Stat Trick automated methodology access was blocked, so its taxonomy remains explicitly unverified rather than inferred from frozen tables.
  - Focused verification passed: `npx vitest --run lib/xg/hardeningAudit.test.ts lib/xg/calibration.test.ts` (7/7); TypeScript passed.
  - Added chronological residual-skill layers with independent shooter-finishing and goalie-save effects, minimum-sample thresholds, and shrinkage. No residual value mutates the baseline shot probability unless a caller explicitly opts into an adjusted output.
  - Added the admin-only `/api/v1/xg/operations` API and `/underlying-stats/xg/operations` page. The endpoint caps the dashboard sample at 1,000 approved shots, paginates joined feature rows in 1,000-row pages across bounded game-ID chunks, and uses exact-count queries for full-table coverage.
  - Live local verification returned `162000` feature rows, `118610` approved predictions, `2772` team aggregates, `42236` player aggregates, and `2922` goalie aggregates. A 500-row team sample passed xGF/xGA symmetry, and the exact-strength benchmark returned `795` 5v5 non-empty-net unblocked shots. The page rendered without a framework overlay and exposed the lease/coverage-contract limitations as partial telemetry.
  - The active/champion legacy registry row has selected features but no `feature_coverage` profile. The dashboard therefore reports feature drift as unavailable instead of fabricating a healthy result; production repair is tracked in NEW 24.0.
  - Read-only paginated coverage reconstruction against the immutable dataset manifest matched `118222/118299` row IDs across `1381` games and 27 selected features. The reconstructed profile SHA-256 is `f537ced619f5751749f1020c267a042504700a6b475c4c087e04500c235a7abf`, with zero blockers and two high-null warnings (`ownerPowerPlayAgeSeconds`, `missReasonBucket`). The 77 absent IDs have SHA-256 `7eb7726ae94c57fe2dd5d1cb4dc6ec5caa974a99aed667f05c805b88e27e0c61` and cluster in historical game `2025020057`; this prevents claiming exact artifact parity without 24.3 disposition.
  - Focused verification passed: nine files / `17` tests covering the xG operations page, operations alerts/reconciliation, calibration, flurry, residual, rebound, tracking, and execution leases; full `npx tsc --noEmit` passed.
  - Applied `20260712214739_xg_execution_leases.sql` plus the ambiguity correction `20260712230000_fix_xg_execution_lease_acquire_conflict.sql`. Production verification proved service-role-only grants, RLS, overlap rejection, owner-only heartbeat/finish, expired-lease takeover, and probe cleanup.
  - Isolated preview `dpl_3q5aujZvUQdwkUFB5RPhSaAhevVF` passed operations health and HTTP `409` overlap rejection before production promotion. Production deployment `dpl_7TgVeKDUhxQjUkur85jeFYKZ7C2c` is READY on `fhfhockey.com`; operations health is complete, feature drift is `ok` for `1000/1000` required rows, production overlap rejection is `409`, probe rows are removed, and the scoped runtime-error scan is empty.

- [x] 24.0 **NEW:** Restore live feature null-rate drift evidence for the active xG registry contract
  - [x] 24.1 Produce or recover an artifact-qualified `feature_coverage` profile for the exact active/champion shot-goal model without changing its prediction math or lifecycle state.
  - [x] 24.2 Persist the verified profile through the registry workflow, then confirm the operations dashboard evaluates at least the required 1,000 scoring rows and reports `ok` or concrete drift issues instead of `unavailable`.
  - [x] 24.3 **NEW:** Reconcile the 77 immutable dataset row IDs absent from current feature version 1 storage before treating the reconstructed 118,222/118,299-row profile as exact; the owner approved coverage option A, preserving the 77-ID hash and reconstructed-profile provenance as an explicit historical evidence exception (2026-07-12).

  Notes:
  - The exact active/champion row `logistic_l2-s20252026-p1-st1-f1-cfg9bac2706` now stores the reconstructed `118222/118299` coverage profile and approved 77-row exception provenance. The semantic profile SHA-256 is `5a69299731237c0ae3cfb960771e6f035ff609b14e45824ffce62aea0f452dd0`; the missing-ID SHA-256 is `7eb7726ae94c57fe2dd5d1cb4dc6ec5caa974a99aed667f05c805b88e27e0c61`.
  - Registry readback preserved approval status, model approval, deployment alias, active flag, and champion flag exactly. No prediction math, artifact bytes, or lifecycle state changed.
  - Both isolated preview and promoted production operations checks evaluate `1000/1000` required scoring rows with feature-drift status `ok`, zero issues, `partial=false`, and no unavailable notes.

- [x] 25.0 **NEW P1:** Remove post-event/outcome leakage exposed by offline tree challengers before accepting challenger evidence or trusting the active champion contract.
  - [x] 25.1 Reconstruct the exact 118,299-row champion contract and quantify the leak: test rows with any `missReasonBucket` are 0/6,484 goals, while `shooterShiftAgeSeconds=0` rows are 1,022/1,042 goals; all three challengers produce implausible AUC near 0.98 (2026-07-12).
  - [x] 25.2 Trace and correct the feature-generation/selection causes without introducing future data, preserving legitimate pre-shot shift context where it is actually available. Removed post-event `missReasonBucket`/`isShortSideMiss` from the baseline contract and made exact shift boundaries prefer the stint ending at the event over the post-stoppage stint beginning at it; 25/25 focused tests and TypeScript pass (2026-07-12).
  - [x] 25.3 Rebuild the frozen dataset and rerun calibrated LightGBM, CatBoost, XGBoost, and state-partitioned candidates on the leak-free contract; record honest held-out metrics and calibration. Exact 118,299-row rebuild has zero forbidden feature keys and no zero-age test shifts; all global/state families completed with validation-only Platt calibration and plausible AUC near 0.81 instead of the leaked 0.98 (2026-07-12).
  - [x] 25.4 Assess the active/champion artifact and production predictions against the corrected contract, then use the production/model-promotion checkpoint before any artifact, lifecycle, or prediction replacement. Registered corrected logistic candidate `cfg2afcf561` as inactive/non-champion and shadow-scored 118,610 separate-version rows; current champion registry/prediction hash remained unchanged and no promotion occurred (2026-07-12).
  - [x] 25.5 **NEW:** Bound large explicit/reference game-ID selection into paginated PostgREST chunks; the first 1,381-game reference request failed before reading data with `UND_ERR_HEADERS_OVERFLOW`. The corrected 200-ID chunked, range-paginated selector completed all 1,381 games and exported the exact frozen row/split counts (2026-07-12).
  - [x] 25.6 **NEW:** Fail closed on unauthenticated model registration and shot-prediction writes. Both isolated preview and promoted production `dpl_8AiEssYqAd3MRt5KDx6M1D9Fx6E6` reject missing credentials with HTTP 401; the rotated secret reaches the guarded handler (2026-07-12).

- [x] 26.0 **NEW:** Materialize the additive flurry-adjusted aggregate surface without overwriting or backfilling raw xG implicitly.
  - [x] 26.1 Add nullable parallel team/player/goalie game and rolling columns plus a writer that fetches bounded flurry metadata and persists raw and adjusted values together. The nullable migration and fail-closed writer are applied/deployed; focused verification passes (2026-07-12).
  - [x] 26.2 Apply the additive migration, regenerate the database schema types, and deploy the aggregate writer through an isolated production release after explicit schema/backfill authorization. Applied production migration `20260712234959`; exact preview `dpl_2ysJPd5AHrLWFf14TjpLew7CVFb4` passed and promoted as production `dpl_3ZeBsnxajE8r8QeirTkdeHA9vDpt` (2026-07-12).
  - [x] 26.3 Run dry-run reconciliation, then a bounded/version-qualified materialization; verify raw aggregates are unchanged, adjusted columns are complete for the selected scope, and operations telemetry reports adjusted-surface completeness/reconciliation. Exact season/model dry run covered 118,610 rows in 41s; production materialization completed in 58s with zero skips, all six adjusted surfaces complete, sampled adjusted symmetry PASS, and all six pre/post raw hashes identical (2026-07-12).
  - [x] 26.4 **NEW:** Fail closed on unauthenticated aggregate materialization requests. Added the standard admin/cron-secret guard after preview verification exposed the missing writer boundary; production rejects missing credentials with HTTP 401 and accepts the rotated secret (2026-07-12).

- [x] 27.0 **NEW:** Replace placeholder richer-rebound outputs with evidence-backed offline head contracts before any production model or schema promotion.
  - [x] 27.1 Define and approve leakage-safe label/grain/window semantics and evaluation gates for rebound danger, goalie freeze/control, and conditional second-chance xG. The versioned contract uses a three-second same-possession window, explicit possession/whistle/period termination, existing high-danger geometry, a public-PBP freeze proxy, target-only next-attempt goal labels, frozen game splits, and minimum coverage/base-rate/AUC gates (2026-07-12).
  - [x] 27.2 Train/evaluate the approved offline candidates against frozen splits; keep each head separate from approved rebound creation and require a later promotion checkpoint. All three logistic heads trained and passed the declared offline gates; test AUC is 0.5828 danger, 0.6174 freeze/control, and 0.6357 conditional second-chance xG. Artifact SHA-256 is `4da2637d1d2f51d99e3edbcac1ca2148268815d8123ac0fec9fbc8e54214f402`; promotion remains false and production row counts are zero (2026-07-12).
  - [x] 27.3 **NEW:** Correct offline bounded-time evidence to use persisted `period_seconds_elapsed` when parser-only `game_seconds_elapsed` is unavailable. The initial fail-closed run trained no heads; the corrected same-period rerun produced the valid 145,050-label artifact, and 12 focused tests plus TypeScript pass (2026-07-12).

- [x] 28.0 **NEW P1:** Fail closed on registry lifecycle query overrides discovered by the dynamic Wave-C audit. An artifact that is not approval-eligible can no longer be labeled approved; activation/champion requests require approved status plus `confirmPromotion=true`, and champion requires active state. The guarded route returns a bounded 409 lifecycle error before any registry write. The corrected candidate and richer-rebound heads remain unpromoted; 13 files/64 tests, TypeScript, scoped lint, and diff integrity pass (2026-07-28).
