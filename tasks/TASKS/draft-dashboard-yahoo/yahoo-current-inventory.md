...
The canonical internal sheet-export reader now uses the shared complete Supabase pagination helper with deterministic `player_name`/`player_key` ordering. This prevents equal-name rows from moving across page boundaries; the lifecycle also requires a value-free `{ok:true,count>0}` receipt and fails before sheet clearing when the canonical view is empty. Focused discovery, writer-permission, and pagination contracts pass `34/34` tests, with TypeScript, scoped ESLint, Prettier, and diff integrity green. This is local NEW 9.5 evidence only; controlled provider/runtime, league-equivalence, Python retirement, and resumable historical-backfill gates remain open. No external state changed.
...
## 2026-07-31 local reader-cutover receipt

Value-free Production parity is exact for the staged readers: `yahoo_nhl_player_map_read` and `yahoo_nhl_player_map_mat` both contain 1,857 rows with digest `d5448d3d5ac68dc47ffbf335f44e71a5`; `yahoo_players_with_normalized_history` preserves all 2,827 latest-player rows and latest-field digest `0844a51501fac22c8a027099cbec6a47`, with normalized history populated for 1,548 players across 496,371 non-null rows. Local read-only consumers now use the canonical views; Production remains on the prior artifact pending one authorized deployment and postdeploy revocation.
...
| Ownership history/backfill | `yahoo_player_ownership_history` plus the atomic writer | Canonical daily history retains stable player/date identity. Applied migration `20260725235646` exposes one read-only security-invoker compatibility view. Named UI/API/export readers now use `yahoo_players_with_normalized_history` with the stable timeline alias; deployed parity and postdeploy retirement are verified, while provider-equivalence and historical backfill remain open. |
