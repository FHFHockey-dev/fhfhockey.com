...
The current scan found: a now-removed public-key fallback in the Python mapping writer; Production's non-atomic v3 latest/history behavior and ambiguous legacy RPC overload; fixed live scheduling plus provider-pagination/deactivation/change-detection gaps in legacy ingestion; legacy/normalized mapping and ownership-history cutover drift; and two stale archived-migration test paths now repaired. Active global configuration/token persistence is consolidated, all active global provider calls share bounded retry telemetry, and legacy Python alternatives are explicitly unscheduled/quarantined. A local service-role-only atomic replacement now preserves omissions and daily ownership/draft identity without swallowed errors, but remains unapplied and runtime-unverified under NEW 9.3.
