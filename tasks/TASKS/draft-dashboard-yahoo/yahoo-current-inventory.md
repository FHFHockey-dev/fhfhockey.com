...
| Ownership history/backfill | `yahoo_player_ownership_history` plus the atomic writer | Canonical daily history retains stable player/date identity. Applied migration `20260725235646` exposes one read-only security-invoker compatibility view. Local named UI/API/export readers now use `yahoo_players_with_normalized_history` with the stable timeline alias; deployed parity and postdeploy retirement remain gated. |
