...
All Yahoo tables have RLS enabled. Credential storage has no browser policy; intended public datasets expose read-only policies, while service-role writers remain separate. Authorized migration `20260723040553_restrict_legacy_yahoo_player_writers.sql` is applied with exact local/remote history alignment: both `upsert_players_batch` overloads and `upsert_yahoo_players_v3` now report `anon=false`, `authenticated=false`, and `service_role=true` for EXECUTE, and the advisor no longer reports browser execution of the `SECURITY DEFINER` writer. Current connected advisors retain the expected pre-cutover materialized-view exposure and identify duplicate Yahoo player/history indexes. The still-unapplied hardening migration reduces browser grants to SELECT, supplies the missing positions policy, proves the unique names identity, preserves the deliberately non-unique map, and removes three indexes exactly covered by primary keys. Separately classified postdeploy migration `20260731022805` requires both security-invoker replacements before revoking browser cache access. NEW 9.6 remains open for ordered Production cutover/parity/revocation/advisor proof.
