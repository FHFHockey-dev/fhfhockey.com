...
- [ ] NEW 9.1 **P0 unauthenticated global Yahoo maintenance routes:** `/api/v1/db/manual-refresh-yahoo-token`, `/api/v1/db/update-yahoo-weeks`, and `/api/v1/db/update-yahoo-players` performed credential reads/provider calls/service-role writes without `adminOnly`. All three now use the shared fail-closed admin/cron boundary locally; scheduled callers already send Vault-backed cron authorization and focused missing-auth/static-inventory tests pass. Keep open until an exact deployment is Production-targeted and value-free missing/invalid/current authorization probes pass without executing a writer (discovered 2026-07-22).
