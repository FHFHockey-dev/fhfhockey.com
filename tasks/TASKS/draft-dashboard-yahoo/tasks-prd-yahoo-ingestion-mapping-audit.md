...
- [ ] NEW 9.5 **P1 incomplete ingestion lifecycle and fixed-season scheduling:** canonical player detail now discovers the newest database-owned NHL game when no maintenance override is supplied, completely and deterministically paginates scoped Supabase keys, fails closed instead of widening to an all-season scan, and emits retry/rate-limit/partial/completeness telemetry with deactivation explicitly disabled. The active player cron still pins `gameId=465`; legacy game/key paths still need complete provider pagination, safe complete-snapshot deactivation/change detection, and the schedule cutover before 3.x/6.1/6.4 can close (updated 2026-07-23).
- [ ] NEW 9.6 **P1 schema/read cutover and security proof drift:** generated/live-era contracts retain normalized and legacy week, mapping, ownership, and RPC surfaces; ownership trends still read `yahoo_players.ownership_timeline`. Current catalog proof also finds broad browser table grants on `yahoo_players`/`yahoo_positions` (effective row mutations remain RLS-denied) and no unique index on `yahoo_names` or `yahoo_nhl_player_map`. Reconcile canonical compatibility views/readers, least-privilege grants, and intended row identities before closing 7.1/8.2/8.4 (updated 2026-07-23).
