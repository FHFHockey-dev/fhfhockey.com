...
- [ ] NEW 9.3 **P1 non-atomic canonical player/history writer drift:** local migration `20260723113533_make_yahoo_player_writer_atomic.sql` defines one security-invoker, service-role-only writer that validates the full batch, atomically writes latest plus idempotent daily ownership/draft history, preserves genuine zero values, and rolls back rather than swallowing errors. The local route uses explicit observed/omitted status and no longer backfills missing observations from prior rows. Static migration/route contracts pass, but application, generated-type/runtime proof, duplicate-date and forced-failure rollback verification remain required before closing 4.2/4.4/4.5/4.8 (updated 2026-07-23).
