...
The authoritative current inventory is `tasks/TASKS/draft-dashboard-yahoo/yahoo-current-inventory.md`; it supersedes this 2025 file inventory where paths, schema, or ownership differ. The implementation list is 29/66 with 37 open. Canonical normalized week ownership, measured non-destructive history retention, and complete Yahoo/NHL mapping segmentation are reconciled alongside the prior credential, identity-review, downstream, and verification work. Read-only production evidence covers 2,827 Yahoo players, 3,555 NHL players, 488,901 normalized ownership-history rows, Yahoo-table RLS/policies/grants, and current advisors. NEW 9.8 records browser execution privileges on three player-writer RPC signatures, including one `SECURITY DEFINER` overload; the local migration/source contract restricts them to `service_role`, while production application and live re-verification remain ask-first. NEW 9.1 also remains open because the prior fail-closed route artifact is target-null rather than Production; atomic writer/history, fixed-season lifecycle, and normalized-reader cutover remain open.
