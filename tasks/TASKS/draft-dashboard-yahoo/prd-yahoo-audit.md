...
The authoritative current inventory is `tasks/TASKS/draft-dashboard-yahoo/yahoo-current-inventory.md`; it supersedes this 2025 file inventory where paths, schema, or ownership differ. The implementation list is 26/65 with 39 open. Current verified ownership/inventory, credential-artifact containment, per-user private-token boundary, one active server-only global credential owner, minimal refresh responses, batched detail extraction, normalized history schema, identity normalization/review, cron correlation, downstream diagnostics/states/contracts, local verification, and archived migration-test fixtures are reconciled. NEW 9.4 closes split active credential ownership through exact-column validation/fail-closed persistence across all three global routes; unscheduled Python alternatives are explicitly compatibility/quarantine paths. The focused helper/auth/missing-config/provider-redaction group passes 2 files/13 tests, full TypeScript and complete lint pass, and no provider/database/credential action occurred. NEW 9.1 remains open because the prior fail-closed route artifact is target-null rather than Production; atomic writer/history, fixed-season lifecycle, normalized-reader cutover, and global-object proof also remain open.
