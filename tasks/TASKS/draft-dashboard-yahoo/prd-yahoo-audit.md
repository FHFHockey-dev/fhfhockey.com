...
The authoritative current inventory is `tasks/TASKS/draft-dashboard-yahoo/yahoo-current-inventory.md`; it supersedes this 2025 file inventory where paths, schema, or ownership differ. The implementation list is 51/71 with 20 open. Canonical normalized week ownership, measured non-destructive history retention, complete Yahoo/NHL mapping segmentation, deterministic evidence-scored identity matching, canonical provenance/review persistence, safe incremental/full-recompute behavior, Production maintenance-route authorization, and repaired atomic writer runtime/cross-runtime contracts are reconciled. Exact commit `b4aeea8f9af6229aa64f75e4b311b1218a5afb4c` is READY/Production as `dpl_4q8GM1wqTbHR3NbnxRxAakdLEvJr`; all three routes pass value-free 401/401/405 probes without a provider or writer. Migrations `20260723040553`, `20260723113533`, and `20260725200808` are applied/history-aligned; the atomic writer is browser-denied/service-role-only with fixed `pg_catalog`, and its bounded latest/omission, duplicate ownership/draft, forced-failure rollback, and zero-residue cohort passes. Generated types and the TypeScript/Python compatibility paths now share the canonical atomic payload and explicit omission semantics. Mapping analysis is side-effect-free unless its exact write opt-in is set; incremental comparison reports removals without deleting them and full recompute fails closed on verified identity conflicts. Fixed-season lifecycle, normalized-reader cutover, broad legacy table grants, and intended identities for `yahoo_names`/`yahoo_nhl_player_map` remain open.
