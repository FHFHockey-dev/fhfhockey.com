...
The authoritative current inventory is `tasks/TASKS/draft-dashboard-yahoo/yahoo-current-inventory.md`; it supersedes this 2025 file inventory where paths, schema, or ownership differ. The implementation list is 47/70 with 23 open. Canonical normalized week ownership, measured non-destructive history retention, complete Yahoo/NHL mapping segmentation, deterministic evidence-scored identity matching, canonical provenance/review persistence, safe incremental/full-recompute behavior, Production maintenance-route authorization, and repaired atomic writer runtime behavior are reconciled. Exact commit `b4aeea8f9af6229aa64f75e4b311b1218a5afb4c` is READY/Production as `dpl_4q8GM1wqTbHR3NbnxRxAakdLEvJr`; all three routes pass value-free 401/401/405 probes without a provider or writer. Migrations `20260723040553`, `20260723113533`, and `20260725200808` are applied/history-aligned; the atomic writer is browser-denied/service-role-only with fixed `pg_catalog`, and its bounded latest/omission, duplicate ownership/draft, forced-failure rollback, and zero-residue cohort passes. Mapping analysis is side-effect-free unless its exact write opt-in is set; incremental comparison reports removals without deleting them and full recompute fails closed on verified identity conflicts. Generated RPC typing/shared cross-runtime row ownership, fixed-season lifecycle, normalized-reader cutover, broad legacy table grants, and intended identities for `yahoo_names`/`yahoo_nhl_player_map` remain open.
...
Current lifecycle update (2026-07-25): the active player-detail route discovers the newest database-owned NHL game unless an explicit maintenance override is supplied, reads scoped keys through deterministic complete Supabase pagination, uses bounded transient-only retry with jitter/`Retry-After`, and emits machine-readable processed/succeeded/failed/omitted/retry/rate-limit/completeness state into the shared durable cron audit. Partial results never deactivate players. Migrations `20260723113533` and `20260725200808` plus route code replace fabricated-zero/previous-row observation fill with explicit omission state and one fail-closed atomic latest/daily-ownership/daily-draft writer. Value-free Production atomicity/idempotency/late-failure proof passes with zero residue and unchanged service-role-only authorization. All active global mutations are method/auth bounded, exact-cron sheet failure output is value-free, and unscheduled mapping/backfill/uniform writers require explicit write opt-in; mapping analysis no longer writes unmatched rows before that boundary. The TypeScript detail pipeline is the sole scheduled uniform-number owner. Deterministic mapping uses exact/manual/alias stages before thresholded fuzzy matching, explicit contextual evidence for ambiguities, canonical versioned FHFH review/provenance persistence, baseline-backed incremental selection, and fail-closed full recompute. The live cron's `gameId=465`, legacy provider-key completeness/deactivation/change detection, generated RPC typing, and shared cross-runtime row-contract separation remain open boundaries.
