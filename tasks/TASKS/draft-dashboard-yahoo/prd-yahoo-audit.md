...
The authoritative current inventory is `tasks/TASKS/draft-dashboard-yahoo/yahoo-current-inventory.md`; it supersedes this 2025 file inventory where paths, schema, or ownership differ. The implementation list is 41/69 with 28 open. Canonical normalized week ownership, measured non-destructive history retention, complete Yahoo/NHL mapping segmentation, deterministic evidence-scored identity matching, canonical provenance/review persistence, Production maintenance-route authorization, and RPC writer containment are reconciled. Exact commit `b4aeea8f9af6229aa64f75e4b311b1218a5afb4c` is READY/Production as `dpl_4q8GM1wqTbHR3NbnxRxAakdLEvJr`; all three routes pass value-free 401/401/405 probes without a provider or writer. Migration `20260723040553` is applied/history-aligned, and live privileges are browser false/service-role true for all three writer signatures. Mapping analysis is side-effect-free unless its exact write opt-in is set. Atomic writer/history, fixed-season lifecycle, normalized-reader cutover, broad legacy table grants, and intended identities for `yahoo_names`/`yahoo_nhl_player_map` remain open.
