...
The authoritative current inventory is `tasks/TASKS/draft-dashboard-yahoo/yahoo-current-inventory.md`; it supersedes this 2025 file inventory where paths, schema, or ownership differ. The implementation list is 31/66 with 35 open. Canonical normalized week ownership, measured non-destructive history retention, complete Yahoo/NHL mapping segmentation, Production maintenance-route authorization, and RPC writer containment are reconciled. Exact commit `b4aeea8f9af6229aa64f75e4b311b1218a5afb4c` is READY/Production as `dpl_4q8GM1wqTbHR3NbnxRxAakdLEvJr`; all three routes pass value-free 401/401/405 probes without a provider or writer. Migration `20260723040553` is applied/history-aligned, and live privileges are browser false/service-role true for all three writer signatures. Atomic writer/history, fixed-season lifecycle, normalized-reader cutover, broad legacy table grants, and intended identities for `yahoo_names`/`yahoo_nhl_player_map` remain open.
...
Current lifecycle update (2026-07-23): the active player-detail route now discovers the newest database-owned NHL game unless an explicit maintenance override is supplied, reads scoped keys through deterministic complete Supabase pagination, uses bounded transient-only retry with jitter/`Retry-After`, and emits machine-readable processed/succeeded/failed/omitted/retry/rate-limit/completeness state into the shared durable cron audit. Partial results never deactivate players. Local migration `20260723113533` and route code replace fabricated-zero/previous-row observation fill with explicit omission state and one fail-closed atomic latest/daily-ownership/daily-draft writer. All active global mutations are method/auth bounded, exact-cron sheet failure output is value-free, and unscheduled mapping/backfill/uniform writers require explicit write opt-in; the TypeScript detail pipeline is the sole scheduled uniform-number owner. The live cron's `gameId=465`, legacy provider-key completeness/deactivation/change detection, migration application, and forced-failure rollback proof remain open boundaries.
