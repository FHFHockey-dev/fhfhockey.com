...
The authoritative current inventory is `tasks/TASKS/draft-dashboard-yahoo/yahoo-current-inventory.md`; it supersedes this 2025 file inventory where paths, schema, or ownership differ. The implementation list is 52/71 with 19 open. Canonical normalized week ownership, complete provider/Supabase player-key pagination, measured non-destructive history retention, complete Yahoo/NHL mapping segmentation, deterministic evidence-scored identity matching, canonical provenance/review persistence, safe incremental/full-recompute behavior, Production maintenance-route authorization, and repaired atomic writer runtime/cross-runtime contracts are reconciled. Exact commit `b4aeea8f9af6229aa64f75e4b311b1218a5afb4c` is READY/Production as `dpl_4q8GM1wqTbHR3NbnxRxAakdLEvJr`; all three routes pass value-free 401/401/405 probes without a provider or writer. Migrations `20260723040553`, `20260723113533`, and `20260725200808` are applied/history-aligned; the atomic detail writer is browser-denied/service-role-only with fixed `pg_catalog`, and its bounded latest/omission, duplicate ownership/draft, forced-failure rollback, and zero-residue cohort passes. Local candidate `20260725220704` adds the still-unapplied service-only versioned key-snapshot boundary; exact provider paging and fail-closed parser regressions pass, while executable SQL/application and controlled provider evidence remain open. Local candidates `20260725235646` and predeploy-safe `20260726000603` add still-unapplied security-invoker ownership/map readers, select-only browser grants, the missing positions read policy, and the proven names identity; focused tests, full TypeScript, zero-error lint, and read-only catalog/advisor evidence pass. The materialized map retains select-only compatibility until code deployment, with final Data API retirement reserved for a separate postdeploy migration. The 1,857-row legacy map remains deliberately non-unique because every full row is distinct despite 771 repeated ID pairs and 12 null Yahoo IDs. Mapping analysis is side-effect-free unless its exact write opt-in is set. Fixed live scheduling, ordered Production application/deployment/parity, postdeploy cache retirement, and explicit legacy-map disposition remain open.
