...
23. Button and toggle styling used by the page must follow the required button/toggle rules defined in `tasks/TASKS/rules/fhfh-styles.md`.
...
- Follow `tasks/TASKS/rules/fhfh-styles.md` loosely for the broader feel.
- The rules in `tasks/TASKS/rules/fhfh-styles.md` that must be followed are the button and toggle styling rules.
...
The owner approved the Phase-1 recommendations on 2026-07-22: 4WK starts expanded, score rank treatment stays on the Score cell plus a restrained edge, desktop vertical remains a named legacy fallback through Phases 2–4, and the already-live path is updated without a second flag. Phase 2 is evidence-complete because the tracked shell already uses the canonical flat dark dashboard surfaces, connected compact command plane, shared header language, striped rows, separators, retained icons/logos, and exact canonical GameGrid button/toggle anatomy. No new token or runtime CSS rewrite was needed. Focused tests pass 3 files/9 tests, full TypeScript passes, and the development route compiles and returns HTTP 200; automated Chromium visual proof is blocked by the documented macOS sandbox and is not claimed. B-GAMEGRID is active at 13/35 with 22 open. Phase 3 remains review-gated.
