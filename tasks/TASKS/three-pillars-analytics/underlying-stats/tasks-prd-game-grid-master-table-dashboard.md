...
- [x] 5.0 Polish the experience across breakpoints and refresh the Player Pickup Table styling to match the new dashboard UI (verified 2026-07-25).
  - [x] 5.1 Mobile/tablet retain the separate stacked collapsible composition with the shared dashboard shell, while desktop master columns preserve readable minimum widths and scroll instead of compressing below their design contract (verified 2026-07-25).
  - [x] 5.2 Opponent Metrics and Four Week Grid already use the shared panel/header, dense zebra/hover table, numeric typography, breakpoint, and internal-scroll recipes; direct Sass compilation verifies both without duplicate style churn (verified 2026-07-25).
  - [x] 5.3 Player Pickup already uses the shared panel shell, table toolbar, dense numeric table, control/focus states, responsive card table, pagination, and scrollbar recipes; direct Sass compilation and static ownership review verify the refresh without changing its logic (verified 2026-07-25).
  - [x] 5.4 The targeted cohort passes sorting, 4WK collapse, Score rendering, sticky/overflow style compilation, minimum-width allocation, and exact stacked/master/legacy-vertical layout policy at the 1023/1024 boundary (verified 2026-07-25).
  - [x] 5.5 Existing `SortableHeaders.test.tsx` now covers the stable width-allocation and breakpoint/orientation seams; the focused three-file group passes 12/12 (verified 2026-07-25).
...
- [x] NEW 2.0 **P2 desktop vertical orientation bypasses the master-table contract:** exact policy coverage proves `<1024px` stays stacked, `>=1024px` horizontal uses the master, and `>=1024px` vertical remains the named `Legacy Vertical` fallback with no second feature flag (verified 2026-07-25).

Phase 5 completed the then-current B-GAMEGRID list at 36/36; the later closed NEW 5 made the runtime-complete source 37/37, and NEW 6 brings the reconciled control source to 38/38. The only Phase-5 runtime change prevents the dense desktop master from shrinking below its canonical column minimums; smaller containers use the existing owned horizontal scroller. No formula, data source, sorting semantics, schedule logic, Player Pickup behavior, dependency, feature flag, build, deployment, or external state changed.
...
- [x] NEW 5.0 **P1 completed GameGrid failed repository TypeScript verification:** `gridLayout` read `isDesktop` before the hook result was declared. The existing unconditional hook call now precedes its first use; the focused regression passes 8/8, scoped lint is clean, and full TypeScript completes without diagnostics (opened and closed 2026-07-25).
- [x] NEW 6.0 **P3 completion-control drift:** reconcile the PRD's stale 36/36 status and the master import's stale 35-row header with the authoritative 37-row source, then import this finding as row 38 without changing runtime scope (verified 2026-07-29).
