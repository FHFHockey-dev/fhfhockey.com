...
- [ ] NEW 1.0 **P0 unauthenticated privileged trend rebuild:** `POST /api/v1/trends/player-trends` accepts arbitrary `startDate`, optional season/player scope, and optional write boundary without any user/admin/cron authorization, then uses the server service-role client to read source views and upsert `player_trend_metrics`. The unlinked browser testing page invokes this contract directly. Require fail-closed authorization, bounded validated scope, and abuse-safe diagnostics; decide whether the testing page becomes authenticated/admin-only, local-only, or read-only before breaking the existing POST contract.
...
- [ ] NEW 3.0 **P1 team-power silent season truncation:** `fetchRowsForSource` in `/api/v1/trends/team-power` reads season-scale NST/WGO team gamelogs without range pagination, although one NHL season exceeds the default PostgREST page cap. Add deterministic ordered pagination for every source, retain exact season/date bounds, and prove category/ranking parity across a multi-page fixture before relying on the endpoint's team percentiles.
