...
- [x] 5.0 Nightly Pipeline Orchestration & Retro Recompute Queue. The canonical TypeScript chain now has a protected resumable worker over the existing priors → window-z → score → transactional distribution/quintile finalization → trend-band stages; independent schedules remain unchanged pending B-CRON-NST NEW 61 (verified locally 2026-07-25).
	- [x] 5.1 Implement the canonical pipeline orchestration. One atomic queue claim owns one bounded stage per request; existing source-advance detection, audited route receipts, queue locking, config-trigger insertion, and cursor advancement supply incremental, logging, locking, and retro behavior without restoring Python persistence (verified locally 2026-07-25).
	...
	- [x] 5.5 Complete runs persist one version/config/season/date/window distribution snapshot and deterministically assign zero-based quintiles only within the explicit score scope. Paged queue runs use one transactional finalizer after the complete score population (verified locally 2026-07-25).
	- [x] 5.6 Exact config activation inserts one unique queued/running recompute identity with a frozen resumable stage cursor in the same transaction (verified locally 2026-07-25).
	- [x] 5.7 The protected TypeScript worker atomically claims with `SKIP LOCKED`, processes one bounded stage, advances or completes idempotently, and records only fixed failure state with capped exponential retry (verified locally 2026-07-25).
	...
	- [x] 5.9 A deterministic full-pipeline fixture proves priors, paged window-z, paged score, finalization, paged trend bands, cursor offsets, exact config identity, terminal completion, and fixed-error retry behavior (verified 2026-07-25).
...
- [ ] NEW 15.0 **P1 canonical version/config provenance drift:** the local canonical repair is complete. CLI-generated migration `20260725223034` preserves reconstructable score provenance, labels only unknown history `legacy_unversioned`, adds first-class version/hash and distribution/quintile state, constrains one active compatible v2 config, removes browser mutation grants, and provides service-only activation, queue claim/advance/backoff, and transactional finalization. Score/prior routes stamp the active config and the protected bounded worker has deterministic full-chain/retry coverage; the version/config group passes 9 files/26 tests and the orchestration group passes 9 files/24 tests plus full TypeScript. Keep the root open only for executable migration rollback/ACL/queue/finalizer proof on an authorized isolated database and exact Production application before code publication; do not treat the unused draft barometer table as canonical (updated 2026-07-25).
