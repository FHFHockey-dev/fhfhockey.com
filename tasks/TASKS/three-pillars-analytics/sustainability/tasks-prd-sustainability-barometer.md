...
- [ ] NEW 15.0 **P1 canonical version/config provenance drift:** fresh local execution now proves data-free legacy-revision bootstrap, exact +1 activation with transactional queue receipt, two-session `SKIP LOCKED` claiming, five-row quintile/distribution finalization, service-only ACLs, rollback-zero residue, and full-chain replay. The canonical score/prior runtime and focused contracts remain green. Keep open only for exact Production application before code publication; do not treat the unused draft barometer table as canonical (updated 2026-07-30).
