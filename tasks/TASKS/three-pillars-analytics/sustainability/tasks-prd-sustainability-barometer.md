...
- [x] 5.0 Nightly Pipeline Orchestration & Retro Recompute Queue. The canonical TypeScript chain now has a protected resumable worker over the existing priors → window-z → score → transactional distribution/quintile finalization → trend-band stages; the Aug 1 B-CRON-NST NEW 61 handoff now verifies the deployed owner/order contract (verified locally 2026-07-25; Production handoff 2026-08-01).
	...
	- [x] 5.10 Document the canonical scheduled route chain, exact retry identity, prerequisite/failure behavior, bounded read/write contracts, the now-satisfied B-CRON-NST NEW 61 ownership contract, Python offline-only boundary, and unimplemented distribution/retro semantics in the existing Sustainability runbook (verified 2026-07-22; Production handoff 2026-08-01).
...
- [x] NEW 15.0 **P1 canonical version/config provenance drift:** fresh local execution proves data-free legacy-revision bootstrap, exact +1 activation with transactional queue receipt, two-session `SKIP LOCKED` claiming, five-row quintile/distribution finalization, service-only ACLs, rollback-zero residue, and full-chain replay. Ordered Production migration `20260725223034` is applied before exact-SHA application publication, closing the final gate without activating a config or executing a writer (completed 2026-07-31).
