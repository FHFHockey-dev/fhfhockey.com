...
Each score rebuild emits named prerequisite, player-load, league-reference, score-build, and persistence timings alongside processed, built, written, chunk, and anomaly counts in both its structured server event and audited response. Extreme detection operates on finite raw window z values before canonical clipping: `|z|>5` persists `extremeFlag`, sorted metric names, and the threshold inside the canonical score component JSON. This preserves the signal without adding a competing draft-table column. The focused observability/score/guardrail/route group passes 4 files/8 tests plus TypeScript and diff integrity, closing 8.1–8.2; random stored-vs-recomputed sampling and nightly seven-day drift remain open under 8.3–8.4.
