...
- [ ] NEW 19.0 **P1 — Make the active aggregate reduction as strict as the rebuilt raw contract.** Replace or harden the remaining `any`-based `processData` path so malformed/null clocks and relationship keys cannot become zero, duplicate player-game rows cannot inflate totals, contradictory mirrored pair observations reject, and player/game/ATOI/shared-percentage semantics are parity-testable through one typed contract. Preserve exact card-stat ownership and document only honest source-specific differences. [Discovered during NEW 10; depends on NEW 10 and precedes NEW 13–16.]
- [ ] NEW 20.0 **P1 — Reconcile the two active `shift_charts` producer contracts and repair incomplete history.** `web/lib/projections/ingest/shifts.ts` can insert rows that omit game type/TOI length/player position/relationship/line-pair fields, while `ingest-projection-inputs` treats any row as complete; establish completeness-aware preflight and one authoritative enrichment contract, prove existing FORGE strength totals remain intact, inventory affected player-games through bounded reads, and gate any production backfill/deployment behind its own approval and rollback evidence. Raw/aggregate readers must expose incomplete rows as partial/error rather than truthful zero. [Discovered during NEW 10; shared B-DRM/FORGE/cron dependency.]
