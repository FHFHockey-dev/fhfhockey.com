...
- [ ] NEW 12.0 **P1 divergent-branch and dirty-overlay reconciliation:** preserve every local and remote non-transient change, reject stale regressions, merge without rewrite, verify the candidate, publish through a normal push, and synchronize the shared checkout without losing ignored/local work. Evidence so far: isolated merge `803f30291` retains the local DRM and 52-commit remote histories; 135 focused tests, TypeScript, formatting, diff integrity, compile, ancestry, and fsck pass; normal push advanced `octoberBranch` from `9ed2f3f8c` to `29a3ab212`. The shared checkout's mixed ref synchronization and post-sync parity proof remain before closure (2026-07-19).
