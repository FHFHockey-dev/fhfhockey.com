...
Exact deleted-path checks find none of the 20 removed files restored. Route/config comparison preserves all approval-gated routes, 48 package scripts, 20 Vercel cron definitions, and every canonical replacement link. READY Production deployment `dpl_Bdu3FhyCTsCbmUzckYEchQHSt2uP` at exact source `589b0241d` returns 200 for seven public shells and 401 for both unauthenticated rolling routes. The separately owned Start Chart API still returns 500 because its Yahoo compatibility views are absent; the verified local recovery remains unpublished, so final integrity row 7.3 and parent 7.0 remain open rather than overstating completion.
