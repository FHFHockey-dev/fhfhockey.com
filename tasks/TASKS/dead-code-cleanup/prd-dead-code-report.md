...
**2026-08-03 local WGO test-contract correction:** The WGO source already resolves each processed season through the canonical NHL Records lineage authority; the full-suite failure was a stale structural assertion searching for the retired catalog call. The test now checks the lineage call before the first `wgo_team_stats` read. Focused authority verification passes `28/28`; full local Vitest passes `642` files / `3,599` tests, with targeted ESLint, Prettier, and diff integrity green. No runtime, data, checkbox, denominator, build, push, deployment, provider, or repair state changed.
...
**2026-08-02 NEW 50 local receipt-contract checkpoint:** The coordinator and direct projection routes now expose explicit termination/resume state, and their audited wrappers opt into a final-audit receipt whose durable row and successful response report `persisted` after the audit insert while a failed insert reports `failed`. Focused local tests pass 29/29 (10 wrapper, 8 coordinator, 11 projection) plus TypeScript; guarded publication and a natural Production receipt remain open.
...
| `web/lib/supabase/Upserts/fetchPPTOIdata.js`, `fetchPowerRankings.js`, `fetchRollingGames.js`, `fetchSKOskaterStats.js`, `fetchSKOyears.js`, `fetchSoSgameLog.js`, `fetchStandings.js`, `fetchWGOgoalieData.js`, `fetchWGOgoalieStats.js`, `fetchWGOskaterStats.js` | Old standalone ingestion scripts/helpers. | CommonJS scripts with hard-coded env/bootstrap patterns; no inbound imports in web runtime. The separately listed `supabaseShifts.js` candidate had no invoker or retained artifact after bounded proof and was deleted on 2026-08-02 under B-DRM NEW 22. | Retain the remaining candidates pending their own exact consumer and owner review; no credential rotation or external action is inferred from the retired shift writer. |
