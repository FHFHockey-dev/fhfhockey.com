...
- NEW 49 records a P1 cross-provider scheduler collision: Vercel schedules the rolling coordinator at the same minute that pg_cron job 308 schedules projection work, while the canonical inventory currently sees only pg_cron. One combined ownership/dependency graph, authoritative writer, rollback plan, zero-overlap proof, and natural ordered run are required before activation.
- NEW 50 records a P1 runtime-contract mismatch: the coordinator permits 270 seconds while the deployed Vercel function cap is 240 seconds. The work must be bounded/split with audit-flush headroom or removed from the natural Vercel schedule, then verified with a nontrivial completed audit.
- NEW 51 records a P2 duplicate-audit contract: job 16 calls the outer audited `update-shifts` adapter, which delegates to an inner audited relationship route. One audit owner plus one-call/one-row regression and bounded/natural proof are required.
