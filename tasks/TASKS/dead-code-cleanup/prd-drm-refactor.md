...
- NEW 56 closes the P2 partial-roster rendering defect: the unified hook owns canonical groups for all display modes, the helper no longer mutates player inputs, and the presentational grid caps and renders forward/defense scopes independently. Zero-line/three-pair and one-line/two-pair regressions plus the full 8-file/114-test DRM suite, TypeScript, formatting, Sass, dual-timezone page tests, lint, diff, and independent-review gates pass. Dormant-wrapper disposition remains separately open under NEW 13.
