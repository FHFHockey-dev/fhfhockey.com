...
- NEW 15 closes the responsive/DatePicker contract: local input/wrapper/calendar hooks style the portal safely, controls and LinePairGrid use explicit four/two/one-column breakpoints, parent grids permit child shrinkage, and minimum matrix tracks overflow only inside the matrix. Real Chromium at 1440/1008/1007/768/390/320 px proves zero body overflow, correct control/card collapse, a themed in-viewport calendar with Escape focus restoration, three-pixel keyboard focus rings, sticky headers, and logical cell navigation after local scrolling. The full 11-file/124-test DRM group, TypeScript, Prettier, bundled-Node-24 Sass, zero-error scoped lint with five pre-existing warnings, and diff integrity pass. NEW 16 query/season/source state remains excluded and open.
