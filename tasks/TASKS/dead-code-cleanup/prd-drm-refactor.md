...
- The first guarded production CLI apply selected only canonical migration `20260720105524` and failed closed before creating any target object or history row because the top-level table lock lacked an explicit transaction boundary under `db push`. NEW 46 owns the minimal `BEGIN`/`COMMIT` repair, static ordering guard, authoritative publication, exact one-file dry run, and production schema/history/security postconditions before the canary may begin.
