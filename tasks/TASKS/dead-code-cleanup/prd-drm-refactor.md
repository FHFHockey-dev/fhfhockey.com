...
### Raw/aggregate parity and producer findings (NEW 19–41)
...
- The legacy standalone writer logged the service-role credential and Supabase URL before self-executing. NEW 22 removes those value-bearing calls without accessing the secret and changes local write failures to stop, but the self-executing entry remains. A value-free invocation/log-retention inventory must precede any conditional credential rotation or entry-point retirement.
...
- Local NEW 25 requires final metadata, nonterminal play coverage, positive unique events, and one terminal; prevalidates shifts before mutation; refreshes every selected source snapshot; exact-replaces PBP plays; verifies metadata and event identities; and leaves PBP/strength scopes incomplete on failure. These writes remain separate PostgREST transactions, so a per-game lock/RPC or shared version manifest, exact content fingerprint, concurrency/rollback proof, production deployment, and bounded history verification remain before closure.
...
- Active cron job 104 still reaches the legacy `update-PbP`/`fetchPbP` writer. It treats any prior row as complete, accepts non-final nonempty payloads, lacks exact play replacement, and logs/continues after write failures. NEW 27 requires the job to be deactivated after a canonical canary or converted to the identical fail-closed lock/hash/RPC contract; an active bypass cannot coexist with a meaningful advisory lock.
...
- The three transactional writers initially labeled same-identity replays idempotent while still rewriting rows and completion timestamps. NEW 31 now requires exact physical owner-scope equality before a no-DML receipt; drift is a non-idempotent versioned repair, and executable rollback/concurrency proof remains mandatory.
...
- The relationship queue's private season lookup selected the prior season after the newest season end date. NEW 33 uses the authoritative latest-started season contract and requires an offseason deployment probe.
- Completed durable operations, client/SQL key grammar drift, and null transition fallthrough could break safe replay. NEW 34 treats completion as a no-op and aligns fail-closed validators, pending executable state-machine proof.
- Mixed producer rows could block derived reads, one-team strength payloads could publish complete, first manifest adoption could retain stale legacy output, and input invalidation could leave unowned rows. NEW 35 filters by strength ownership, enforces two teams, invalidates on first adoption, and cleans debris in the same transaction, pending behavioral branch/canary proof.
- Independent review found two duplicate SQL column declarations that would have prevented the additive migration from compiling. NEW 36 removes them, adds a uniqueness guard, and keeps the P0 open until a clean supported-baseline branch applies the whole migration.
- A relationship algorithm bump could remain permanently skipped because queue identity ignored the stored algorithm. NEW 37 adds version-aware queue selection and keeps deployment/runtime evidence explicit.
- Active job 16 still invokes legacy `update-shifts?action=all` before canonical input, while the Vercel coordinator omits the transactional relationship stage. NEW 38 owns promotion of the canonical relationship writer and strict input → relationship → derived order, including a derived-status gate and legacy-owner rollback.
- The active coordinator's explicit dates/`maxGames` disable input's durable mode, derived continuation is response-only, and date-range keys do not carry a failed operation into tomorrow's run. NEW 39 requires one stable backlog/retry identity across every scheduled stage and next-day recovery proof.
- Transaction clients hash freshly fetched source payloads, but SQL accepts caller-asserted 64-hex source hashes without checking the latest immutable Gamecenter snapshot/version. NEW 40 binds immutable provenance inside the locked transaction before normalized exact-scope claims can close.
- Early 400/405 exits in the canonical relationship route could leave the finally-written cron audit marked successful. NEW 41 locally derives audit failure from every final HTTP status at or above 400 and has focused runtime coverage; deployment/natural evidence remains.
- The resulting producer graph is: authoritative immutable snapshot identity and exact normalized scope (`NEW 25/27/29/40`) → canonical locked relationship replacement and active schedule (`NEW 20/26/38`) → atomic player/team/goalie derived publish (`NEW 23/28`) → stable cross-day durable backlog (`NEW 24/39`) → branch rollback/concurrency/ACL/audit evidence → bounded repair/canary → natural cron evidence → legacy-owner retirement. Status and cursor advancement cannot precede the corresponding game transaction.
