...
- NEW 56 closes the P2 partial-roster rendering defect: the unified hook owns canonical groups for all display modes, the helper no longer mutates player inputs, and the presentational grid caps and renders forward/defense scopes independently. Zero-line/three-pair and one-line/two-pair regressions plus the full 8-file/114-test DRM suite, TypeScript, formatting, Sass, dual-timezone page tests, lint, diff, and independent-review gates pass; exact checkpoint `fb4e5fd778954546db174ff546dd099752cf9324` is published with fresh local/tracking/live equality. Dormant-wrapper disposition remains separately open under NEW 13.
- NEW 57 closes the P2 render-time caller-mutation defect: every total-TOI sort operates on a copy, and the exact caller-order regression passes.
- NEW 58 closes the P2 accessible-meaning defect: only clocks with seconds `00`–`59` are announced; missing and malformed values, including `20:99`, remain `N/A ATOI` in tooltip and screen-reader descriptions.
- NEW 59 closes the P2 logical-focus defect: roving, focused, hovered, ref, and header-highlight ownership now uses stable row/column player IDs. A focus-then-rerender TOI reorder regression preserves the same logical focused cell, exactly one tab stop, and continued keyboard navigation.
- NEW 14 closes the selector/matrix accessibility remediation through native labeled selectors and buttons, pressed states, semantic grid/row/header/cell structure, equivalent value descriptions, logical roving keyboard navigation, pointer/focus parity, and visible focus/tooltip behavior. Exact checkpoint `46400a3639557b6742af14a6f55339abcfa3d393` is published with fresh local/tracking/live equality. NEW 15 remains separately open for responsive and DatePicker styling/browser work.
- NEW 15 closes the responsive/DatePicker contract: local input/wrapper/calendar hooks style the portal safely, controls and LinePairGrid use explicit four/two/one-column breakpoints, parent grids permit child shrinkage, and minimum matrix tracks overflow only inside the matrix. Real Chromium at 1440/1008/1007/768/390/320 px proves zero body overflow, correct control/card collapse, a themed in-viewport calendar with Escape focus restoration, three-pixel keyboard focus rings, sticky headers, and logical cell navigation after local scrolling. The full 11-file/124-test DRM group, TypeScript, Prettier, bundled-Node-24 Sass, zero-error scoped lint with five pre-existing warnings, and diff integrity pass.
- NEW 16 closes deterministic URL/season/source ownership. A local Pages-Router-safe query-state hook hydrates direct URLs after mount, updates synchronously, preserves adjacent keys, and restores popstate without letting SSR defaults erase dates. Strict sanitizers cover team, mode, adjacent-year season, season type, timeframe, source, venue, opponent, and dates; exact historical-season lookup returns bounded 400/404/503 states and never falls back to current season. Raw reads now accept the selected season and reject conflicting rows, while raw QA selection idles aggregate cards honestly. Production-mode Chromium restored the exact Edmonton 2024–25 playoffs/raw/custom URL and dates, cleaned a fully malformed URL to safe current-season defaults, and rendered the default route with zero console errors or TOI overlay. The consolidated 14-file/146-test group, full TypeScript, production build/static generation, live HTTP 200, and browser gate pass.
- NEW 60 closes the P1 active-data availability defect. Raw and aggregate readers transactionally exclude a whole game when appearance TOI exceeds game length or relationship TOI exceeds either appearance, never cap/fabricate values, preserve strict row/scope/clock/identity/mirror validation, and expose exact partial coverage while retaining every usable game. The production-backed PHI season now renders its cards and matrix with an explicit 76-row partial notice, no runtime overlay/console error, and zero body overflow. Full DRM 11 files/129 tests plus TypeScript, Prettier, Sass, zero-error scoped lint with five pre-existing warnings, and diff integrity pass. The initial bounded inventory remains 98 appearance/game-length offenders across 77 games/29 teams; producer/history correction remains under NEW 20/25–29 and outside this local reader slice.
