...
### Raw/aggregate parity and producer findings (NEW 19–42)
...
- NEW 48 local remediation is branch-verified: exact raw-PBP-bound roster fallback resolves before relationship calculations; canonical client serialization and raw-hash preflight fail before RPC; the version advances to `shift_relationship_materializer_v3_position_bound`; and a column-specific, execute-revoked invoker trigger rejects null, noncanonical, missing-primary, cross-type, duplicate-display, and type-conflict writes while allowing unrelated updates to historical invalid rows. The focused 5-file/57-test and broader 42-file/321-test groups, full TypeScript, scoped lint, independent re-review, service-role trigger proof, zero residue, and branch deletion pass. Publication, production migration/deployment, and the bounded canary/replay remain open.
