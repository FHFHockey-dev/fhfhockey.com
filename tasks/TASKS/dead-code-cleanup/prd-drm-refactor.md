...
- `shift_charts` still has two incompatible active producer classes: the TypeScript route and legacy JavaScript upsert both write DRM relationship/game/position fields, while projection-input ingestion can insert only strength totals and basic metadata and considers any existing row complete. The TypeScript full writer also models and emits line/pair assignments as strings even though the generated table contract is numeric. Such rows cannot prove raw DRM pair semantics and can also make later full enrichment appear unnecessary. NEW 20 owns completeness-aware producer/preflight/type reconciliation, bounded affected-row inventory, and separately approved production backfill/deployment. Until then the bounded raw reader deliberately includes nullable partial producer rows and reports whole-game partial coverage; the exact aggregate query filters to the selected non-null season/game type and therefore cannot expose those omitted rows as partial until NEW 20 repairs producer/history completeness. This source-specific difference is documented rather than disguised as parity.
