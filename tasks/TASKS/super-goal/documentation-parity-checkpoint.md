...
**2026-08-04 actionable-open gate audit:** The current unchecked-row partition has 51 actionable opens: A-AUTH 6, A-CRON-EMAIL 3, A-GDL 11, B-YAHOO 7, A-3P 2, A-SUST model 3, B-SUST-AUD 7, and B-DRM 12. The six fenced rule examples remain non-actionable. Every open implementation row is held by an explicit credential/provider, historical-repair/backfill, Production/natural-run, or owner-authorized external-evidence gate; C0042/7.0, C0047/7.5, and DRM NEW 50 are closed by the receipts above. No unapproved external action occurred.
...
- A read-only parser found all 76 imported source paths represented in the canonical master. Counts are exact at `4,954/5,011` raw; no source/master checked/total mismatch was found. The six fenced `generate-tasks.mdc` format examples remain explicitly non-actionable reference rows.
- The mechanical master roll-up is `5,101/5,176`; imported actionable parity is `4,954/5,005` with 51 open. Historical checkpoint paragraphs remain append-only and are not treated as current totals.
