...
| B-DRM | Date Range Matrix refactor | Maintenance/product | Pair repaired; committed partial implementation; 14/67 rows complete after architecture/baseline, playoff/value, async request-state, deterministic fail-closed aggregate reader, owner-approved Option-A/Custom remediation, exact-scope/stale-reset card remediation, typed raw-source rebuild, strict aggregate/raw reduction parity, and raw boundary hardening. NEW 20/23–25 and 30–37 have substantial local producer/transaction/auth/receipt/CAS/season/replay/integrity/compile/algorithm-aware containment with focused tests and full TypeScript, but no branch/runtime claim | P0 NEW 22/30/36, P1 NEW 20/23–29/31–33/35/38–40, and P2 NEW 13–16/34/37/41 remain open for value-free credential disposition, executable supported-baseline migration and transactional proof, canonical input → relationship → derived scheduling, stable cross-day backlog recovery, immutable-source provenance, exact scope/history repair, deployed auth/audit truth, bounded canary, and natural evidence |
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-drm-refactor.md` | task list | B-DRM | missing pair repaired; 67 rows, 14 architecture/correctness/request-state/aggregate-reader/Option-A/exact-card-scope/stale-reset/typed-raw/strict-reduction/boundary rows verified; NEW 20/23–40 remain open across credential, transaction, history, schedule, backlog, provenance, migration/runtime, auth, and audit gates; P2 NEW 13–16/34/37/41 remain |
