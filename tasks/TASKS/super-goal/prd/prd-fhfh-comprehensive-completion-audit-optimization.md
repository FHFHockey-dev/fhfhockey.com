...
|---|---|---|---|---|
| A-AUTH | Authentication and user settings | Platform | Wave A active; 189/207 rows, 18 open | Shared auth/entitlement foundation; callback, expired-session recovery, P1 autofill-exposed credential containment, canonical provider/manual-config documentation, and production confirmation/recovery Inbox receipt are reconciled; received-link/provider/PKCE gates remain open |
| A-CRON-EMAIL | Cron audit email failures | Operations | Wave A active; 55/62 rows, 7 open. The audit-payload parent is closed by deployed wrapper provenance plus a natural `update-shift-charts` success/HTTP-200 row on the exact scheduled route. Cross-route auth inventory freezes 59 active HTTP jobs / 52 routes, 19 admin-or-cron, one exact-cron-only, 32 unprotected, and all browser/internal callers after projection input/derived hardening; root enforcement remains route-by-route. P1 season-writer/WGO/missing-secret auth boundaries are verified locally; P0 finalized after stable redeploy, 59/59 literal-to-Vault conversion, 61/61 activation parity, 47/47 green probes, missing/old rejection and current authorization, canonical-one/candidate-zero cleanup, executable-helper removal, and honest provider/cron-history/WAL/backup/log retention documentation | Operational visibility and credential boundary; reconcile B-CRON-NST |
| A-GDL | GDL suite ingestion | Data ingestion | Wave A active; 72/85 rows, 13 open. Canonical provenance/RPC migration `20260716112909`, replacement-branch compatibility, production apply, one exact two-team Gamecenter probe, and local four-receiver processor-error containment through NEW 21 are verified. NEW 20 is closed by four distinct value-free Vercel/applet rotations, unchanged-source READY Production redeploy `dpl_43DPkUc3bVS2CVitR7vdiduf3CzJ`, and exact four-route retired/replacement `401/400` proof. IFTTT support replied but could not associate the case with the originally supplied identity; read-only signed-in settings prove a different account identity, and the owner has started the value-free identity-only follow-up tracked by NEW 22. No invalidation evidence exists, and billing disclosure is not authorized. Promotion/events remain paused because payload repair NEW 14 remains, approved remove/reconnect failed to retire the prior provider key (NEW 13/15/16/17/18/22), and the disconnected podcast trigger drifted (NEW 19) | Shared lines identity/data foundation |
...
| B-CLEAN | Code cleanup/warning reduction | Maintenance | Active at 13/44; cache-history containment `NEW 8.0–8.5`, checkpoint prevention `NEW 9.0`, generated-build publication containment `NEW 10.0–10.3`, three-file forward-only bytecode cleanup `NEW 11.0`, and no-rewrite branch/worktree reconciliation `NEW 12.0` are verified. The clean post-reconciliation baseline has local/tracking/live parity at `8ad7329e9`, zero ahead/behind, empty index/worktree before the resulting control-only diff, supported migration-root/archive separation, and retained recovery stash | Return to broad warning cleanup after active feature stabilization |
...
| B-DRM | Date Range Matrix refactor | Maintenance/product | Pair repaired; committed partial implementation; 25/74 rows complete after architecture/baseline, reader/card/raw remediation, NEW 31 physical no-DML replay, NEW 36 supported-public-baseline compilation, NEW 40 immutable snapshot binding, NEW 42 named-conflict repair, NEW 34/43 durable terminal-state proof, NEW 44 exact ACL remediation, NEW 45 exact FK-index remediation, NEW 46 canonical CLI transaction-boundary remediation, NEW 30 deployed route-auth proof, and NEW 47 frozen inventory reconciliation. The complete additive migration compiled/applied on the authorized data-less branch; four corrected functions retain eight named and zero positional conflict targets; all three writer replays, late rollback, stale pruning/version rejection, A→B→A correction/removal, current-version downstream publication, and real two-session races pass. Exact forced-RLS/browser-denial/service-role/raw-trigger/index contracts survived zero-residue cleanup; candidate-specific advisors are acceptable; the branch is deleted and confirmed absent. Exact published commit `d8e98d9570989e4840a9172980ccb3abf36fb9ce` repaired the first fail-closed production CLI boundary; canonical production history version `20260720105524`, 3 tables, 5 indexes, 4 triggers, 7 functions, narrow ACL/RLS, 5,680 valid heads, empty pre-canary state, and zero candidate advisor warning/error now pass. The exact-SHA isolated deployment is READY with no custom alias movement, and all three route boundaries plus value-free audit groups pass. NEW 48 is locally and branch-remediated with exact roster/raw-hash binding, algorithm-v3 canonical client guards, legacy-compatible relationship-column trigger enforcement, focused 5-file/57-test and broader 42-file/321-test groups, runtime invalid/valid/service-role/legacy-update proof, zero residue, acceptable advisors, and confirmed branch deletion | P0 NEW 22, P1 NEW 20/23–29/32–33/35/38–39, and P1 NEW 48 plus P2 NEW 13–16/37/41 remain open for credential disposition, canonical scheduling/order, stable backlog recovery, exact scope/history repair, publication/production canary, truthful natural audits, and UX/integration work |
...
Reorder update (2026-07-20): A-GDL NEW 22 is a same-root identity-only follow-up to NEW 18 after the provider could not associate the case with the originally supplied support identity. It briefly interrupts B-DRM only long enough for the owner-started value-free reply; after send verification, NEW 18/22 return to the meaningful-checkpoint bookmark and B-DRM resumes while support acts. Billing disclosure remains separately unapproved.

B-DRM dependency update (2026-07-21): NEW 30/31/36/40/42/44–47 and the temporary-branch rollback/concurrency/security/audit gate are complete. The disposable branch was cleaned and deleted. The first exact production CLI push failed closed before target/history mutation; exact published commit `d8e98d9570989e4840a9172980ccb3abf36fb9ce` added the full-file transaction, and the canonical migration now passes every production history/schema/head/security/advisor/log postcondition. A READY exact-SHA isolated deployment has no custom alias movement; all three materialization routes pass missing/invalid/current header-only auth and value-free audit proof, and the frozen inventory matches the hardened classification. Remaining execution resumes at production-gated NEW 20/22–29/32–33/35/37–39/41, with NEW 29 retaining broader history/xG/TOI compatibility and B-CRON-NST NEW 60 separately owning the authoritative `analytics` baseline.
...
| `tasks/TASKS/auth-user-settings-platform/tasks-prd-auth-user-settings-platform.md` | task list | A-AUTH | 207 checkbox rows; 189 checked and 18 open |
...
| `tasks/TASKS/cron-operations/tasks-prd-cron-nst-audit-remediation.md` | task list | A-CRON-EMAIL / B-CRON-NST / C-OPS | 64 checkbox rows; 56 checked and 8 open |
...
| `tasks/TASKS/lines-gdl-ingestion/tasks-prd-gdl-suite-ingestion.md` | task list | A-GDL | 85 checkbox rows; 72 checked and 13 open after authoritative-root rollout, four-receiver downstream-error containment, failed provider retirement/removal recovery, provider account-identification mismatch, post-restore trigger drift, and completed receiver-secret containment |
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-cleanup-tasks.md` | task list | B-CLEAN | missing pair repaired; 44 current rows after cache-history, checkpoint-prevention, generated-build publication, older-bytecode, and branch-reconciliation discoveries; 13/44 verified |
| `tasks/TASKS/dead-code-cleanup/tasks-prd-dead-code-report.md` | task list | B-DEAD | missing pair repaired; 35 generated rows |
| `tasks/TASKS/dead-code-cleanup/tasks-prd-drm-refactor.md` | task list | B-DRM | missing pair repaired; 74 rows, 25 architecture/correctness/request-state/aggregate-reader/Option-A/exact-card-scope/stale-reset/typed-raw/strict-reduction/boundary/named-conflict/durable-terminal/ACL/FK/production-migration/deployed-auth/inventory rows verified; NEW 20/22–29/32–33/35/37–39/41/48 remain across credential, transaction, production, history, schedule, backlog, provenance, position integrity, and audit gates; P2 NEW 13–16/37/41 remain |
...
|---|---|---|---|
| A-AUTH Authentication/settings | `auth-user-settings-platform/prd/prd-auth-user-settings-platform.md` | `auth-user-settings-platform/tasks-prd-auth-user-settings-platform.md` | Direct; 189/207 reconciled with 18 received-link/provider evidence rows open |
