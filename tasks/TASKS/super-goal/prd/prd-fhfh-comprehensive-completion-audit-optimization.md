...
| B-CLEAN | Code cleanup/warning reduction | Maintenance | Active at 44/71. Prior cache/history/generated-state containment remains verified. The baseline/ownership and corrected semantic-accessibility slice closes base 1.0–2.3, verification 6.1/6.4, and NEW 18/24/27–39 except the separately open 19–23/25–26: native headers retain `aria-sort`, labeled buttons own sorting, all three day-switch views use a stable included-state name, arrow direction matches the ARIA state, repeated Enter/Space is canceled without extra callbacks, clipped/forced-colors focus remains visible, and parent propagation is render-safe. Two focused files/six tests, strict changed-scope 0/0 lint, full TypeScript, Sass, formatting/diff integrity, independent no-finding re-review, and clean-browser layout/focus interaction pass. The stable comparable result is five ARIA-role warnings to zero; the final 1,500-file/129-warning inclusive run is not presented as a direct comparison with the earlier 1,497-file/132-warning snapshot. NEW 7/19–23/25–26 and remaining hook/image/noise/toolchain/integrated-verification work stay open | Continue dependency order at stale directives or wrapper/fallback correctness; hold NEW 20 for a material toolchain strategy checkpoint |
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-cleanup-tasks.md` | task list | B-CLEAN | missing pair repaired; 71 current rows after containment and warning/accessibility discoveries; 44/71 verified after final three-view switch/arrow/repeat re-review; NEW 7/19–23/25–26 and broader hook/image/noise/toolchain/integrated work remain open |
