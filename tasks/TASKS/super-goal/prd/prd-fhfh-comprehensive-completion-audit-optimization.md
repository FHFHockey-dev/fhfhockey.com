...
| B-CLEAN | Code cleanup/warning reduction | Maintenance | Active at 62/99 with 37 open. NEW 55/56 close the bounded parent TeamDashboard season-state and stale-request defects through a backward-compatible named query-status hook, truthful pending/terminal/no-query states, explicit-season bypass, and one effect-local owner across all eleven awaits plus catches/finalization. Two focused files/13 tests, full TypeScript, zero-error scoped lint with the identical pre-existing parent hook warning, Prettier, diff integrity, and independent review pass. NEW 57–67 remain separately dependency-ordered alongside NEW 7/19–20/26/40–42/45/50 and broader image/hook/noise/toolchain/integrated work | Continue TeamDashboard NEW 57/58 before NEW 59/67, then shared schedule-hook NEW 60/61/65 before active Forge consumer verification; keep NEW 45/50 separate and hold NEW 20 plus dependent NEW 40/41 for a material lint/toolchain strategy checkpoint |
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-cleanup-tasks.md` | task list | B-CLEAN | missing pair repaired; 99 current rows after containment, warning/accessibility/suppression, evidence correction, wrapper/fallback, migrated-logo/TeamLeaders, and TeamDashboard season/request ownership work; 62/99 verified; NEW 7/19–20/26/40–42/45/50/57–67 and broader image/hook/noise/toolchain/integrated work remain open |
