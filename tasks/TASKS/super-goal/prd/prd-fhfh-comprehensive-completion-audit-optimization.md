...
| A-GDL | GDL suite ingestion | Data ingestion | Wave A active; 72/84 rows, 12 open. Canonical provenance/RPC migration `20260716112909`, replacement-branch compatibility, production apply, one exact two-team Gamecenter probe, and local four-receiver processor-error containment through NEW 21 are verified. NEW 20 is closed by four distinct value-free Vercel/applet rotations, unchanged-source READY Production redeploy `dpl_43DPkUc3bVS2CVitR7vdiduf3CzJ`, and exact four-route retired/replacement `401/400` proof. The IFTTT acknowledgment has a provider reference but no invalidation evidence; a verified credential-free same-thread reply supplies the reproduction and requests all-historical-key revocation plus current-key-only confirmation. The latest owner checkpoint reports no further provider response. Promotion/events remain paused because payload repair NEW 14 remains, approved remove/reconnect failed to retire the prior provider key (NEW 13/15/16/17/18), and the disconnected podcast trigger drifted (NEW 19) | Shared lines identity/data foundation |
...
| B-CLEAN | Code cleanup/warning reduction | Maintenance | Active at 12/44; cache-history containment `NEW 8.0–8.5`, checkpoint prevention `NEW 9.0`, generated-build publication containment `NEW 10.0–10.3`, and three-file forward-only bytecode cleanup `NEW 11.0` are verified. `NEW 12.0` has a verified normal-push reconciliation candidate published at `29a3ab212`; shared-checkout mixed synchronization/parity remains | Complete `NEW 12.0`, then return to broad warning cleanup after active feature stabilization |
...
| B-DRM | Date Range Matrix refactor | Maintenance/product | Pair repaired; committed partial implementation; 14/51 rows complete after architecture/baseline, playoff/value, async request-state, deterministic fail-closed aggregate reader, owner-approved Option-A/Custom remediation, exact-scope/stale-reset card remediation, typed raw-source rebuild, strict aggregate/raw reduction parity, and raw boundary hardening. NEW 20/23–25 local producer/timeline/exact-scope/cursor/PBP containment passes 13 files/166 tests plus full TypeScript/lint/format/diff checks | P1 NEW 20/23–25 remain open for transactional/versioned production rollout, authoritative materializer/history repair, durable cron continuation, and natural evidence; P0 NEW 22 remains open for value-free external/log inventory and conditional credential disposition before duplicate-owner and UX work |
...
Owner-approved temporary reorder (2026-07-18): A-GDL NEW 18 remains an open P0 external-provider dependency with bounded checks only at meaningful initiative checkpoints. Its held applet/event/email/data descendants do not block non-overlapping work. The previously omitted A-DRAFT-RANKER initiative was reconciled before queued local A-AUTH documentation work because the mandatory wave order prioritizes active/in-progress initiatives. A meaningful mailbox/remote checkpoint then exposed authoritative PR #352; the exact rollout evidence closes Wave-A Phase 7 and reclassifies only the five already-deferred Phase 8 rows to `B-DRAFT-RANKER-P8`. After the next safe-work audit found no locally closable A-AUTH/Cron row without an external or breaking-contract boundary, B-DRM moved ahead of not-started Wave-B scopes because its target tree contains committed partial implementation rather than merely planned work. Its internal graph is baseline reconciliation → active aggregated playoff/value correctness → async request-state safety → deterministic aggregated pagination/date ownership → explicit last-N semantics → displayed-card canonical-scope alignment and stale-consumer reset → typed raw-source rebuild → strict aggregate/raw reduction parity → producer completeness/backfill → duplicate-owner/wrapper disposition → accessibility/responsive polish → integrated verification; the first eight stages are complete and producer completeness/backfill is next. No new production or provider mutation was performed by either reconciliation. A-AUTH's remaining production/provider evidence, A-CRON/B-CRON's production or breaking-contract gates, and A-3P's missing external odds key remain explicit higher-order dependencies.
...
| `tasks/TASKS/cron-operations/tasks-prd-cron-nst-audit-remediation.md` | task list | A-CRON-EMAIL / B-CRON-NST / C-OPS | 63 checkbox rows; 56 checked and 7 open |
...
| `tasks/TASKS/draft-ranker/tasks-prd-draft-ranker.md` | task list | A-DRAFT-RANKER / B-DRAFT-RANKER-P8 | 54 rows; 49 checked and five deferred Phase 8 rows open |
...
| `tasks/TASKS/lines-gdl-ingestion/tasks-prd-gdl-suite-ingestion.md` | task list | A-GDL | 84 checkbox rows; 72 checked and 12 open after authoritative-root rollout, four-receiver downstream-error containment, failed provider retirement/removal recovery, post-restore trigger drift, and completed receiver-secret containment |
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-cleanup-tasks.md` | task list | B-CLEAN | missing pair repaired; 44 current rows after cache-history, checkpoint-prevention, generated-build publication, older-bytecode, and branch-reconciliation discoveries; 12/44 verified |
| `tasks/TASKS/dead-code-cleanup/tasks-prd-dead-code-report.md` | task list | B-DEAD | missing pair repaired; 35 generated rows |
| `tasks/TASKS/dead-code-cleanup/tasks-prd-drm-refactor.md` | task list | B-DRM | missing pair repaired; 51 rows, 14 architecture/correctness/request-state/aggregate-reader/Option-A/exact-card-scope/stale-reset/typed-raw/strict-reduction/boundary rows verified; local NEW 20/23–25 containment is verified but held open at the production transaction/history/cursor boundary; P0 NEW 22 and P2 NEW 13–16 remain |
...
| A-DRAFT-RANKER / B-DRAFT-RANKER-P8 Account-backed personal NHL Draft Ranker | `draft-ranker/prd-draft-ranker.md` | `draft-ranker/tasks-prd-draft-ranker.md` plus launch-runbook source checklist | Direct; repaired; Phase 7/DR-072 complete, 49/54 overall, and the five deferred Phase 8 rows explicitly mapped to Wave B |
