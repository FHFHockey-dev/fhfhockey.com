...
| B-DRM | Date Range Matrix refactor | Maintenance/product | Pair repaired; committed partial implementation; 22/72 rows complete after architecture/baseline, reader/card/raw remediation, NEW 31 physical no-DML replay, NEW 36 supported-public-baseline compilation, NEW 40 immutable snapshot binding, NEW 42 named-conflict repair, NEW 34/43 durable terminal-state proof, NEW 44 exact ACL remediation, and NEW 45 exact FK-index remediation. The complete additive migration compiled/applied on the authorized data-less branch; four corrected functions retain eight named and zero positional conflict targets; all three writer replays, late rollback, stale pruning/version rejection, A→B→A correction/removal, current-version downstream publication, and real two-session races pass. Exact forced-RLS/browser-denial/service-role/raw-trigger/index contracts survived zero-residue cleanup; candidate-specific advisors are acceptable; the branch is deleted and confirmed absent. The exact reviewed implementation/control checkpoint `fe8d039b9cb601bb0422a180c3a8103da9e94ff8` is published on `octoberBranch` after the 23-file/170-test, TypeScript, lint, diff/parity, scope, and fast-forward gates passed. The first production CLI apply failed closed before history/object mutation; NEW 46 owns the explicit transaction-boundary correction and production postconditions | P0 NEW 22/30, P1 NEW 20/23–29/32–33/35/38–39/46, and P2 NEW 13–16/37/41 remain open for credential disposition, production auth/deployment, canonical scheduling/order, stable backlog recovery, exact scope/history repair, bounded canary, truthful natural audits, and UX/integration work |
...
B-DRM dependency update (2026-07-21): NEW 31/36/40/42/44/45 and the temporary-branch rollback/concurrency/security/audit gate are complete. The disposable branch was cleaned and deleted. The first exact production CLI push failed closed before target/history mutation because the migration's top-level table lock lacked an explicit transaction; NEW 46 now precedes the bounded canary. Remaining execution resumes at production-gated NEW 20/22–30/32–33/35/37–39/41/46, with NEW 29 retaining broader history/xG/TOI compatibility and B-CRON-NST NEW 60 separately owning the authoritative `analytics` baseline.
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-drm-refactor.md` | task list | B-DRM | missing pair repaired; 72 rows, 22 architecture/correctness/request-state/aggregate-reader/Option-A/exact-card-scope/stale-reset/typed-raw/strict-reduction/boundary/named-conflict/durable-terminal/ACL/FK rows verified; NEW 20/22–30/32–33/35/37–39/41/46 remain across credential, transaction, production, history, schedule, backlog, provenance, auth, and audit gates; P2 NEW 13–16/37/41 remain |
