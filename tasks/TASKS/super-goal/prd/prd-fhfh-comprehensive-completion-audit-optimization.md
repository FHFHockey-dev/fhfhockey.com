...
| B-CLEAN | Code cleanup/warning reduction | Maintenance | Active at 18/49; cache-history containment `NEW 8.0–8.5`, checkpoint prevention `NEW 9.0`, generated-build publication containment `NEW 10.0–10.3`, three-file forward-only bytecode cleanup `NEW 11.0`, no-rewrite branch/worktree reconciliation `NEW 12.0`, all 13 tracked Supabase CLI-state removals `NEW 13.0/15.0`, four-file Playwright result cleanup `NEW 14.0`, seven-file generated log/audit cleanup `NEW 16.0`, and the Yahoo historical-log sensitivity/history decision `NEW 17.0` are verified. Every physical ignored CLI/log/audit copy remains locally available; the value-free Yahoo-log review found no high-confidence or current local credential and did not justify a history rewrite. The clean post-reconciliation baseline has local/tracking/live parity at `8ad7329e9`, zero ahead/behind, empty index/worktree before the resulting control-only diff, supported migration-root/archive separation, and retained recovery stash | Return to broad warning cleanup after active feature stabilization |
...
Reorder update (2026-07-21): A-GDL NEW 22 is a same-root identity-only follow-up to NEW 18 after the provider could not associate the case with the originally supplied support identity. The owner sent the candid value-free clarification with the existing settings screenshot and Sent Items verification passed; NEW 18/22 therefore return to the meaningful-checkpoint bookmark while B-DRM resumes and support acts. Billing disclosure remains separately unapproved.
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-cleanup-tasks.md` | task list | B-CLEAN | missing pair repaired; 49 current rows after cache-history, checkpoint-prevention, generated-build publication, older-bytecode, branch-reconciliation, Supabase CLI internal-state, Playwright result-artifact, generated log/audit, and historical-log sensitivity discoveries; 18/49 verified |
| `tasks/TASKS/dead-code-cleanup/tasks-prd-dead-code-report.md` | task list | B-DEAD | missing pair repaired; 35 generated rows |
| `tasks/TASKS/dead-code-cleanup/tasks-prd-drm-refactor.md` | task list | B-DRM | missing pair repaired; 85 rows, 40 architecture/correctness/request-state/aggregate-reader/base-pagination/stable-hook/explicit-state/wrapper-inventory/Option-A/exact-card-scope/stale-reset/typed-raw/strict-reduction/boundary/named-conflict/durable-terminal/ACL/FK/production-migration/deployed-auth/inventory/first-adoption/position-integrity/repeated-control/calendar-validation/canonical-scheduled-route/exact-mode-budget/independent-partial-roster rows verified; NEW 20/22–29/32–33/37–39/41/49–51 remain across credential, transaction, production, history, schedule, backlog, provenance, and audit gates; P2 NEW 13/15–16/37/41/51 remain |
