...
| B-CLEAN | Code cleanup/warning reduction | Maintenance | Active at 50/77 with 27 open. Prior containment, semantic accessibility, and stale-suppression/count correction remain verified. Base 4.1 plus NEW 22/23 now close the bounded wrapper/stats-logo defect: explicit dimensions or `fill` are enforced, keyed state resets for either source, fallback is one-attempt and caller-safe, decorative alt remains intact, and both stats paths use the tracked FHFH asset. The five-file/20-test group, full TypeScript, zero-finding scoped lint, targeted formatting/diff integrity, React review, and independent review pass; optimized compilation/type validation pass before isolated `/lines` prerender reaches its absent Supabase bindings. NEW 25 and NEW 45 separately retain TeamLeaders and residual nonexistent-logo scope. NEW 7/19–20/25–26/40–42/45 and remaining image/hook/noise/toolchain/integrated-verification work stay open | Continue at base 4.2/NEW 25; hold NEW 20 and dependent NEW 40/41 for a material lint/toolchain strategy checkpoint |
...
| `tasks/TASKS/dead-code-cleanup/tasks-prd-cleanup-tasks.md` | task list | B-CLEAN | missing pair repaired; 77 current rows after containment, warning/accessibility/suppression, final evidence-correction, and fallback discoveries; 50/77 verified after the bounded wrapper/stats-logo closure; NEW 7/19–20/25–26/40–42/45 and broader image/hook/noise/toolchain/integrated work remain open |
