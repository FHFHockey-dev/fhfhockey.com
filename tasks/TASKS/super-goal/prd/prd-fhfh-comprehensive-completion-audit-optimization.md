...
| A-GDL | GDL suite ingestion | Data ingestion | Wave A active; 70/75 rows, 5 open. Canonical provenance/RPC migration `20260716112909`, replacement-branch compatibility, production apply, one exact two-team Gamecenter probe, and exact READY/current deployment are verified. NEW 12 closes raw downstream-error relay before activation; only unresolved-name email proof and three controlled IFTTT/end-to-end event checks remain | Shared lines identity/data foundation |
...
| `tasks/TASKS/lines-gdl-ingestion/tasks-prd-gdl-suite-ingestion.md` | task list | A-GDL | 75 checkbox rows; 70 checked and 5 open after authoritative-root production rollout and downstream-error containment |
