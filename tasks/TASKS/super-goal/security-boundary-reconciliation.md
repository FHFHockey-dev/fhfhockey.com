...
| Views, functions, and schema exposure | Ordinary public readers are security-invoker; privileged/intermediate surfaces are browser-denied/service-only, and the two unused tombstoned RPCs were removed after one-release zero-use monitoring. The unified materialized aggregates are moved behind non-exposed `internal_stats` storage with canonical select-only wrappers. Direct browser calls to protected surfaces return denial/406 in the recorded value-free checks. | No new public privileged function/view was found. The exact legacy-RPC final drop is applied and catalog-verified; retained Yahoo-cache postdeploy/advisor disposition remains a distinct gate. |
