...
**2026-08-04 projection/accuracy plus cron natural closure:** Natural `run-projection-v2` at `2026-08-04T10:12:01.295463Z` and `run-projection-accuracy` at `11:30:02.941178Z` are POST/HTTP 200 with `success=true`, `partial=false`, and no stale failure status, closing C0008/2.0, C0012/2.4, and C0013/2.5. Children 7.1–7.5 are checked: focused route/component tests and TypeScript pass, the preview contract suppressed both email paths, and the first post-publication natural `daily-cron-report` at `2026-08-03T21:15:07.800997Z` is non-preview GET/HTTP 200 with `success=true`, `warnMissingAudit=0`, and `jobsMissingLast=0`. Value-free response-shape checks found no missing-job or audit-gap findings; remaining partial failures are retained as real route/data/provider observations, including the intentional rolling-games 410 and Yahoo provider 500s. C0042/7.0 and C0047/7.5 are closed; no email was sent or manual route invoked.
