...
- The scheduled-route authorization inventory currently resolves 59 active HTTP jobs to 52 route files: 45 admin-or-cron, one exact-cron-only, and six unprotected. The latest bounded cohort protects 14 scheduled writers whose callers already carry cron/admin authorization; the corrected caller inventory is 5 browser-admin, 13 internal-server, 4 mixed, and 30 cron-only after adding both directly imported WGO handlers.
