...
- Production `public.get_skater_game_score_by_limit(bigint, integer)` is an `anon`-owned, PUBLIC-executable, mutable-search-path `SECURITY DEFINER` routine with no proven repository/function/view consumer. Replaying a no-owner dump would recreate it under `postgres` and broaden the effective privilege boundary, so NEW 57 excludes it from the baseline and defers production mutation to a separate value-free consumer/public-contract checkpoint.
