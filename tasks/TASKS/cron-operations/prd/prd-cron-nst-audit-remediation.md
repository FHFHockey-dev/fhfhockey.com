...
- NEW 60 supersedes only NEW 36's claim of complete exposed-schema reproducibility. Production contains 16 relations and one routine in the exposed `analytics` schema, while the supported baseline reconstructs only `public`; repository namespace creation appears only in two standalone ratings scripts, while analytics objects are scattered across ratings, view, function, historical root-migration, and retained-history sources. Current repository sources define only 12 relations plus one routine and Git history retains another deleted materialized-view definition, so they cannot reconstruct the exact production catalog without extraction. Local `supabase/config.toml` exposes only `public` and `graphql_public`, while hosted branch behavior proves `analytics` remains in the effective Data API schema list. A later authorized data-less branch therefore has no `analytics` namespace, PostgREST repeatedly returns 503/`PGRST002`, and the baseline's public SKO wrapper is runtime-broken because it calls the missing analytics routine. The B-DRM migration itself remains SQL-healthy; only a temporary branch-local empty schema may be used to finish its type/cleanup proof, and that diagnostic schema must be removed before branch deletion. Recommended option A preserves the hosted contract through exact production-catalog extraction, config-as-code, credential-free authoritative analytics migration, exact production/branch catalog and ACL parity, healthy REST/type generation on a fresh supported branch, guarded publication, and separately authorized production migration-history reconciliation without destructive replay. Option B removal requires value-free consumer inventory, public-wrapper retirement, and breaking-contract approval.
...
- NEW 56/57's value-free consumer inventory is complete without invoking either routine or reading either body. Current repository and Git-history review finds no caller; catalog inspection finds zero inbound routine/view/trigger/cron dependencies; retained `pg_stat_statements` shows zero execution-like calls since its 2026-06-03 reset and reports no fingerprint eviction; and bounded 24-hour Supabase API/Postgres plus Vercel runtime-log samples contain zero matches. Function-call counters are non-evidence because production tracking is disabled. These negatives materially reduce current-use likelihood but cannot exclude an unpublished, opaque, or pre-reset external client, so they do not authorize a breaking change.
- Production `public.update_all_wgo_skaters()` remains executable by PUBLIC/`anon`/`authenticated`/`service_role` and retains the excluded credential-bearing body. The similarly named active cron uses the authenticated HTTP route, not this RPC. NEW 56 therefore remains open for explicit public-contract approval before revoke/replace/removal, zero-residue verification, and rollback proof.
- Production `public.get_skater_game_score_by_limit(bigint, integer)` remains an `anon`-owned, PUBLIC/`anon`/`authenticated`/`service_role`-executable, mutable-search-path `SECURITY DEFINER` routine. Git history proves its former UI caller was replaced on 2025-04-16 by `get_skater_game_scores_for_season`. Replaying a no-owner dump would recreate the legacy routine under `postgres` and broaden the effective privilege boundary, so NEW 57 remains excluded from the baseline and separately approval-gated before repair, revoke, replacement, or removal.
