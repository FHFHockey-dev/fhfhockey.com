...
- NEW 60 exact extraction now passes a fresh Colima/Docker replay of all 21 migrations, exact 14-relation/one-routine/zero-policy catalog proof, local public/analytics type generation, empty-data runtime probes after materialized-view refresh, and a one-migration down/reapply rehearsal. The materialized view is created unpopulated because the supported baseline excludes row data. Production tracking reconciliation remains separately authorization-gated and must not replay destructive DDL.
- NEW 61 is the shared A-SUST scheduler-ownership dependency. The local combined inventory reconciles 64 active canonical pg_cron jobs with 20 Vercel jobs and proves same-domain priors and window-z writers overlap at 10:42 and 10:43 UTC. No schedule changes are made by the inventory; one authoritative owner or non-overlapping leased scope, exact rollback metadata, deployed zero-concurrency and ordered-handoff proof, truthful audits, and one natural ordered run remain required.
...
- NEW 63 records three pre-existing compile-invalid routines surfaced by current local `db lint`; none belongs to the pending migration cohort. Exact caller classification and a separate repair/tombstone migration are required.
...
- The scheduled-route authorization inventory resolves 59 active HTTP jobs to 52 route files: 51 admin-or-cron, one exact-cron-only, and zero unprotected. The final six-route local cohort preserves the session-bearing browser caller and exact-cron-bearing internal callers; the direct Yahoo sheet-sync owner is corrected to the shared ingestion-lifecycle helper. Caller shapes remain 5 browser-admin, 13 internal-server, 4 mixed, and 30 cron-only. Publication, value-free denial/current-auth canaries, bounded runtime-error evidence, and a scheduled audit observation remain open.
...
- The first natural nonzero production report after NEW 50 succeeded with 64/64 inventory and successful email delivery, but its otherwise healthy response contained one self-referential missing-audit warning for `daily-cron-report`. NEW 58 owns the narrow current-self-run grace and subsequent natural zero-warning proof; parents 3.0 and 5.0 remain open until that evidence exists.
- The same report records zero failing/missing/unknown last job states but three partial-success jobs totaling 25 failed rows. NEW 59 now exposes bounded `displayName`/`failedRows` pairs in the report warning payload for value-free identity recovery and reconciliation; the two 24-hour failure audits are intentional authorization probes and are not evidence of failed scheduled work.
- NEW 56/57's value-free consumer inventory is complete without invoking either routine or reading either body. Current repository and Git-history review finds no caller; catalog inspection finds zero inbound routine/view/trigger/cron dependencies; retained `pg_stat_statements` shows zero execution-like calls since its 2026-06-03 reset and reports no fingerprint eviction; and bounded 24-hour Supabase API/Postgres plus Vercel runtime-log samples contain zero matches. Candidate migration `20260730190000` now preserves both exact signatures while replacing the unsafe bodies/owner/definer/search-path/grant contracts with credential-free security-invoker tombstones, browser denial, and service-only monitoring. The rows remain open for isolated executable proof, exact predeploy authorization/application, browser denial, one-release zero-use monitoring, and separately approved final removal; no historical unsafe body may be printed or restored.
