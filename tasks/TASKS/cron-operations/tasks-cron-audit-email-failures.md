...
- [ ] NEW 9.0 **P1 inbound scheduled-route authentication boundary:** Audit and enforce the intended inbound authentication contract across scheduled HTTP routes. All 59 active HTTP jobs / 52 routes are now protected locally: 51 use `adminOnly` and one destructive internal route is exact-cron-secret-only. The final six-route cohort preserves the session-bearing browser caller and exact-cron-bearing internal callers. Keep open for grouped publication, value-free missing/invalid 401 and current-auth safe-validation canaries, bounded runtime-error evidence, and the next scheduled audit observation.
  - [x] NEW 9.1 Inventory every active HTTP job and public/manual caller, define fail-closed exceptions and rollout/rollback behavior, then propose the cross-route enforcement boundary before changing production contracts. Evidence updated 2026-07-30: the static contract resolves all 59 active HTTP jobs to 52 route files and freezes 51 admin-or-cron / 1 exact-cron-only / 0 unprotected modes, while preserving 5 browser-admin, 13 internal-server, 4 mixed, and 30 cron-only caller shapes. No public unauthenticated target exists; grouped publication, 401/current-auth canary/audit observation, and affected-batch rollback remain required.
