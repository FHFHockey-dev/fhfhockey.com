...
- [ ] NEW 9.0 **P1 inbound scheduled-route authentication boundary:** Audit and enforce the intended inbound authentication contract across scheduled HTTP routes. The machine-checked pre-rollout inventory now proves 59 active HTTP jobs / 52 routes: 17 use `adminOnly`, one destructive internal route is exact-cron-secret-only, and 34 remain explicitly unprotected. No public unauthenticated exception is approved; migrate the frozen set route-by-route while preserving every inventoried browser/internal caller.
  - [x] NEW 9.1 Inventory every active HTTP job and public/manual caller, define fail-closed exceptions and rollout/rollback behavior, then propose the cross-route enforcement boundary before changing production contracts. Evidence (2026-07-18): the static contract resolves all 59 active HTTP jobs to 52 route files, freezes 17 admin-or-cron / 1 exact-cron-only / 34 unprotected modes, and names all non-cron consumers as 5 browser-admin, 11 internal-server, 4 mixed, and 32 cron-only routes. No public unauthenticated target is allowed; the documented rollout requires per-route caller propagation plus 401/200 tests/canary/audit observation and rolls back only the affected wrapper/caller batch. Focused inventory/audit suites pass 10/10, TypeScript and formatting pass, and no route behavior or production state changed.
