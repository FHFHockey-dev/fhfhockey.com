...
- [ ] NEW 9.0 **P1 inbound scheduled-route authentication boundary:** Audit and enforce the intended inbound authentication contract across scheduled HTTP routes. The current 2026-08-01 Production readback has 54 active HTTP jobs / 48 routes, all protected locally by `adminOnly`; the exact-cron-only sheet-sync route and the five overlapping retired writers are inactive. Keep open for grouped publication, value-free missing/invalid 401 and current-auth safe-validation canaries, bounded runtime-error evidence, and the next scheduled audit observation.
