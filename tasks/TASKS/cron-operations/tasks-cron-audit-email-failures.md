...
- [ ] NEW 9.0 **P1 inbound scheduled-route authentication boundary:** Audit and enforce the intended inbound authentication contract across scheduled HTTP routes. The 2026-07-12 credential rotation proved that cron commands send `Authorization: Bearer <CRON_SECRET>`, but the shared audit wrapper and most route handlers do not validate it; only the destructive Yahoo-to-Sheets sync currently enforces `CRON_SECRET`.
