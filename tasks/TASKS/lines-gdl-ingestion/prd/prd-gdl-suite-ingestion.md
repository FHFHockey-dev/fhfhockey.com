...
35. Restoring a previously disconnected applet must preserve both its disconnected state and exact trigger/action selections. Any provider-selected default drift must be recorded and corrected without leaving the applet active unintentionally.
    - Owner-approved podcast exception (2026-07-30): the already-disconnected podcast applet is retired instead of repaired. Its drifted trigger remains inactive historical configuration; it must not be reconnected, promoted, or used as GDL evidence. This does not relax the exact-contract requirement for any retained applet.
...
38. Provider-support account lookup must use identity verified from the signed-in IFTTT account without persisting the address in repository artifacts. Account identification is complete after support found the account and escalated the historical-key retirement question to engineering. Keep the retirement/downstream hold until direct retired-key rejection and current-key authorization are proved.
...
3. Live IFTTT applet, deployed-secret, controlled-event, and email verification remain manual checkpoints and do not authorize exposing credentials in task artifacts or chat. Production SQL/provenance rollout is complete. Read-only provider inspection found a credential-containment checkpoint and payload drift that must be repaired before production events. IFTTT support has now identified the account and escalated historical-key retirement to engineering; the owner clarified that the request is precautionary with no known leak. Provider-side invalidation and direct retired-key rejection/current-key authorization remain unproven.
