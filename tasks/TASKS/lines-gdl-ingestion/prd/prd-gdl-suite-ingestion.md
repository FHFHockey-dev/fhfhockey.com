...
3. Live IFTTT applet, deployed-secret, controlled-event, and email verification remain manual checkpoints and do not authorize exposing credentials in task artifacts or chat. Production SQL/provenance rollout is complete. Read-only provider inspection found a credential-containment checkpoint and payload drift that must be repaired before production events.
