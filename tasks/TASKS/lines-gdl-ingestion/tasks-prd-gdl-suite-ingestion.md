...
- [ ] NEW 18.0 **P0 Webhooks remove/reconnect does not retire the prior key:** IFTTT archived all five applets, accepted a fresh service connection with a distinct current key, and allowed the prior four-Connected/one-disconnected state to be restored, but the pre-reconnect key still authorized a unique bounded no-listener event after reconnection. Preserve the restored state, submit a value-free provider-support request for account-level invalidation, and require prior-key rejection plus current-key authorization before any downstream action. This is additional remediation evidence for the existing provider-retirement root finding, not a second severity count. Evidence (2026-07-18): the provider acknowledgment includes a provider reference and invites diagnostic evidence but does not confirm account-level invalidation or historical-key rejection. A credential-free reply was sent in the exact thread with the bounded regenerate/retest/remove/reconnect sequence and explicit requests to revoke all historical keys, confirm every old generation invalid, confirm current-key-only authorization, and identify any required account/service reset. Sent Items contains exactly one matching reply with the three core confirmation requests. At the latest meaningful initiative checkpoint, the owner reports no further IFTTT response beyond that acknowledgment. Owner-approved scheduling: keep this row bookmarked as a held external dependency, check the exact thread only at meaningful initiative checkpoints, and continue non-overlapping super-goal work without touching NEW 14/19 or held applet/event/email/data paths. Keep this row open until provider-side invalidation is proven by prior-key rejection and current-key authorization.
