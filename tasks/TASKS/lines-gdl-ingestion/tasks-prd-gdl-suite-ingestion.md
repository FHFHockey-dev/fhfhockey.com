...
- [ ] NEW 18.0 **P0 Webhooks remove/reconnect does not retire the prior key:** IFTTT archived all five applets, accepted a fresh service connection with a distinct current key, and allowed the prior four-Connected/one-disconnected state to be restored, but the pre-reconnect key still authorized a unique bounded no-listener event after reconnection. Preserve the restored state, submit a value-free provider-support request for account-level invalidation, and require prior-key rejection plus current-key authorization before any downstream action. This is additional remediation evidence for the existing provider-retirement root finding, not a second severity count. Evidence (2026-07-21): support first could not locate the account from the originally supplied identity; after the owner's identity-only clarification, Ryan found the account and assigned the historical-key validity issue to engineering. The owner then answered Ryan's risk question value-free: there is no suspected leak, and the refresh is precautionary after sharing with this Codex collaboration. No provider invalidation or rejection evidence was supplied. NEW 22 closes only account identification without storing any address or billing value. Owner-approved scheduling remains in force: keep this row bookmarked as a held external dependency, check the exact thread only at meaningful initiative checkpoints, and continue non-overlapping super-goal work without touching NEW 14/19 or held applet/event/email/data paths. Keep this row open until engineering confirms provider-side invalidation and direct prior-key rejection/current-key authorization pass.

- [x] NEW 19.0 **P2 disconnected podcast trigger drift after restore:** the owner-approved disposition retires this unrelated podcast applet rather than repairing or activating it. Existing provider evidence proves the restored card remains disconnected; its drifted Spotify selection is therefore quarantined as inactive historical configuration. Do not reconnect, correct, promote, or use it for GDL evidence, and keep the four production GDL/CCC applets plus every credential/event action independently held behind NEW 13–18 (closed evidence-only 2026-07-30).
