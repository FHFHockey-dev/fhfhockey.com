...
- [ ] NEW 22.0 **P0 IFTTT support account-identification mismatch (same retirement root):** reply in the exact provider thread using only the value-free, read-only-verified alternate IFTTT account identity and ask support to retry account lookup plus account-level historical-key invalidation before requesting billing evidence. Do not record the account address in repository artifacts or disclose cardholder, last-four, expiration, charge-date, amount, Apple, or Google billing details without separate explicit owner authorization. Keep NEW 13/15/16/17/18 and every downstream applet/promotion/event/email/alias/reprocess/data action held until support identifies the account and prior-key rejection/current-key authorization are directly proved. Evidence (2026-07-20): the signed-in settings page proves the expected username, a different account email, Pro state, and a linked Google account; the owner-authorized identity-only reply was reviewed, rewritten, and saved with the existing screenshot attachment, but no send or provider action is yet verified.
