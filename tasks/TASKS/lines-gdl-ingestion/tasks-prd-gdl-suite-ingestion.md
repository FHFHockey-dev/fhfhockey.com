...
- [ ] 10.0 Validate end-to-end rollout
  ...
  - [ ] 10.4 Create and test the three IFTTT applets with one controlled event per GDL account.
    - Readiness evidence (2026-07-16): the three source routes, exact header-secret variable names, account-specific URLs, JSON ingredient body, and raw-first retry behavior are reconciled in code/runbook. The ignored local configuration has all three secret variable names set; Production name presence and IFTTT account/applet state remain intentionally uninspected until authorization.
