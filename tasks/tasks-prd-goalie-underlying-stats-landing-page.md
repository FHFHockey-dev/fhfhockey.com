...
- [ ] 2.0 Create the dedicated goalie routing and API surface while preserving the shared underlying player-summary pipeline
  - [ ] 2.1 Create the dedicated landing route at `pages/underlying-stats/goalieStats/index.tsx`.
  - [ ] 2.2 Create the dedicated detail route at `pages/underlying-stats/goalieStats/[playerId].tsx`.
  - [ ] 2.3 Add dedicated goalie API wrappers for landing, detail, and chart reads under `/api/v1/underlying-stats/goalies`.
  - [ ] 2.4 Implement goalie-specific request parsing and URL builders so the goalie surface has stable, shareable route contracts independent of the player route namespace.
  - [ ] 2.5 Ensure the dedicated goalie read routes delegate to shared aggregation logic rather than duplicating underlying metric computation.
