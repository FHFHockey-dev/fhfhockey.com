...
## September 12 production activation follow-up

- Owner authorized the production fix after a paid customer could discover leagues but Live Sync was unavailable. Purchase activation and Yahoo linking were confirmed on the same FHFH account; different billing/login emails were unrelated.
- Production previously lacked the Yahoo rollout/season settings and a hosted polling schedule. The completed private-league rehearsal is recorded in `yahoo_draft_sessions` with a final worker heartbeat on September 12 at 19:02 UTC.
- Hosted execution now uses `/api/internal/yahoo-live-draft` every minute through Vercel Cron, authenticated with the existing `CRON_SECRET`. It reuses the coordinator and database leases in bounded 65-second windows with four-session batches, returning immediately when no active/predraft sessions exist. This replaces the need for a continuously running separate Node host; the command-line worker remains available for controlled rehearsals. Provider backoff and entitlement/ownership checks are unchanged. Scheduler delays remain visible through existing freshness indicators.
- Production settings: `YAHOO_LIVE_DRAFT_ENABLED=true`, `YAHOO_LIVE_DRAFT_ROLLOUT_STAGE=authenticated`, `YAHOO_LIVE_DRAFT_PROVIDER_VALIDATED=true`, `YAHOO_LIVE_DRAFT_SEASON=2026`, `YAHOO_LIVE_DRAFT_TARGET_SEASON_ID=20262027`. Current Draft Pro eligibility is still required server-side.
- Rollback: set `YAHOO_LIVE_DRAFT_ENABLED=false` and redeploy; sessions, picks, purchases, and local drafts remain stored. Do not rotate or expose the cron secret as part of this rollout.
