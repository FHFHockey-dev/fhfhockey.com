...
The full Saved Drafts release-candidate unit suite subsequently passed at `acc95703d`: 750 files and 4,349 tests passed, with 3 tests skipped and no failures (`/tmp/draft-pro-saved-drafts-rc-full.log`, exit0). This supersedes the historical initial-RC unit failure counts above. The skips are unchanged ESPN/Fantrax live checks and the optional Python golden-vector fixture; they are not verified. Full browser acceptance remains pending: its real two-context run found loss of the imported source's selected weight after restoration, despite retained private rows. That defect must be corrected and verified before Saved Drafts activation.
