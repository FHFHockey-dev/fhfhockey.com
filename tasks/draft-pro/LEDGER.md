...
Latest accepted estimate: **71% overall / 95% initial paid release**. Saved Drafts implementation, real-service API and full release-candidate unit checkpoints are approved; W12 browser acceptance remains open. Its reported custom-source defect was withdrawn after correcting test observation timing. W09 scenarios and W10 reports are queued. The user redeemed the banked reset; latest sampled weekly usage was **14%**. The figures below at ~13:00 are historical, not the current estimate.
