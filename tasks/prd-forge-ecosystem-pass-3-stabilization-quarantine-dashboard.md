...
- `run-rolling-forge-pipeline.ts` and `rollingForgePipeline.ts` now define the newer stage model with stage-8 behavior described accurately as legacy start-chart materialization plus accuracy work.
- The start-chart read layer is now aligned to canonical `forge_player_projections`, but the legacy `player_projections` materializer still exists as bounded transitional debt.
...
| `web/lib/rollingForgePipeline.ts` | module entry | Declares canonical stage order | Defines rolling-to-FORGE pipeline model | High | `HEALTHY` | `SAFE TO RUN` | Stage 8 now accurately describes legacy start-chart materialization plus accuracy, reducing false confidence from spec drift | Keep as authoritative stage contract |
...
| `web/pages/api/v1/db/update-start-chart-projections.ts` | `/api/v1/db/update-start-chart-projections` | Transitional legacy start-chart materializer | Reads rolling rows, line combos, team ratings, goalie priors; writes `player_projections` | High | `QUARANTINED` | `NEEDS VERIFICATION` | Still writes legacy `player_projections`, so it remains outside the canonical FORGE skater-storage story | Keep only as explicitly transitional materialization until callers are retired or the route is replaced |
...
| `QUARANTINED` | `web/pages/api/v1/db/update-start-chart-projections.ts` | `/api/v1/db/update-start-chart-projections` | Transitional downstream materializer still writes legacy `player_projections` | Start-chart reader no longer depends on `player_projections`, but this route still writes them and is still modeled as a downstream stage | FORGE migration corrected the read layer first, leaving legacy materialization as bounded debt | Legacy table can continue to consume operator attention and conceal whether any real consumer still needs it | No | `replace` or `delete` once remaining callers are mapped; until then keep explicitly transitional |
...
14. Map remaining `player_projections` consumers, then retire or replace `update-start-chart-projections.ts` so legacy start-chart materialization no longer survives beside canonical FORGE skater storage.
...
- Should `update-start-chart-projections.ts` survive as a deliberate transitional materializer, or can the repo now retire `player_projections` entirely after consumer verification?
