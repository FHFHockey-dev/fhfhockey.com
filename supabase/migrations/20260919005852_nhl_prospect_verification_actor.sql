-- Record the automated verification actor required by the identity registry.
create or replace function public.import_nhl_prospect_identity(payload jsonb, roster_season bigint)
...
begin
  ...
  if nhl_id is not null then
    insert into public.fhfh_player_external_identities(fhfh_player_id, provider, external_player_id, context_key, is_primary,
      match_method, match_confidence, verification_status, source_provenance, verified_at, verified_by_system)
    values(identity_id, 'nhl', nhl_id::text, 'global', true, 'official_nhl_id', 1, 'verified',
      jsonb_build_object('url', payload->>'sourceUrl'), checked, 'system:official_nhl_identity_import_v1')
