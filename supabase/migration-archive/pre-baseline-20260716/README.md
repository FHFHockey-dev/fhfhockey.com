...
The baseline was generated from the 2026-07-16 schema-only production `public` dump (source SHA-256 `90e0b4a9067c1451243c7d976eba478b16dd811a9cee81079619d14969e70970`). It normalizes pg_dump client meta-commands, `CREATE SCHEMA public`, and only the 16 legacy NST sequences with no `OWNED BY` dependency so repeated remote resets remain executable while all 41 owned sequences stay strict. It also appends the exact production public-schema relation/function/schema ACL statements, excluding the four credential-bearing functions that are intentionally absent from the baseline and the 12 `supabase_admin` default-privilege statements that a supported `postgres` migration cannot replay; the 12 `postgres`-owned public defaults remain in the baseline. Supabase rejects unavailable legacy extension-version pins on new branches, so the baseline declares the required provider extension inventory without version pins and explicitly casts `cron_job_report.jobname` to PostgreSQL `name` to preserve the production public-view contract when a new branch receives pg_cron 1.6.4 instead of production's unavailable 1.4-1 package. Its generated SHA-256 is `37d2239a049600452847cd57fcf9e42b04853a8b7ca5877cb29d74230072d740`.
